...
// 🔥 KONAČAN SPISAK OD 13 PDF DOKUMENATA (Novi "Commercial Production Protocol" je prvi Hero!) 🔥
const pdfDocuments = [
...
// -------------------------------------------------------------
// Dodali smo prop "isHero" koji signalizira da li je ovo prvi dokument na listi
const V8PdfViewerCard = ({ doc, onOpenFullScreen, isHero }) => {
  ...
  return (
    // Ako je isHero true, na velikim ekranima zauzima 2 kolone (lg:col-span-2)
      ...
      <div className="flex justify-between items-center border-b border-orange-500/30 pb-3 md:pb-4 mb-4 gap-3 flex-wrap shrink-0">
        ...
        <div className="flex items-center gap-2 md:gap-3 shrink-0">
          ...
          <div className="flex items-center gap-1.5">
            {/* DUGME ZA OTVARANJE U NOVOM TABU */}
            <a 
              href={safePath}
              target="_blank"
...
// -------------------------------------------------------------
// 2. GLAVNA KOMPONENTA SA GLOBALNIM FULL-SCREEN SISTEMOM I PAGINACIJOM
// -------------------------------------------------------------
const V8Decks = () => {
  const [activeIndex, setActiveIndex] = useState(null);
  const [fsZoom, setFsZoom] = useState(1);
  const [visibleCount, setVisibleCount] = useState(3); // 🔥 Početno se vide 3 (1 Hero preko obe kolone + 2 ispod)
  ...
  const handleToggleView = () => {
    if (visibleCount < pdfDocuments.length) {
      setVisibleCount(prev => Math.min(prev + 2, pdfDocuments.length)); // Dodaje po 2 nova dokumenta (simetrija u gridu)
    } else {
      setVisibleCount(3); // Resetuje na početnih 3 (1 Hero + 2 ispod)
  ...
  return (
    <section id="v8-decks-section" className="w-full py-12 md:py-16 relative z-10">
      ...
      {/* KARTICE (GRID) - Renderuje vidljivi broj, ali je PRVA KARTICA Hero klasa */}
      ...
      <div className="flex justify-center mt-12 w-full">
        ...
        >
          {visibleCount < pdfDocuments.length ? (
            <>
              <span>SEE MORE</span>
              <ChevronDown size={20} strokeWidth={3} className="group-hover:translate-y-1 transition-transform" />
            </>
          ) : (
            <>
              <span>CLOSE</span>
              <ChevronUp size={20} strokeWidth={3} className="group-hover:-translate-y-1 transition-transform" />
            </>
          )}
        </button>
      </div>
