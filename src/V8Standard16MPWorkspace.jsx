...
const V8Standard16MPWorkspace = () => {
  ...
  const ucitajSlike = async (selectedFiles) => {
    ...
    if (files.length > 0) prvaSlikaZaPoredjenje = files[0];
    else prvaSlikaZaPoredjenje = noveSlike[0];
    ...
    for (let file of noveSlike) {
       try {
         ...
         if (fileFormat === referentniFormat) filtriraneSlike.push(file); 
         else odbijenoZbogFormata = true; 
       } catch (e) {}
    }

    if (odbijenoZbogFormata) alert(`Pokušali ste da ubacite slike različitog formata! U ovom batchu su dozvoljene samo slike u formatu ${referentniFormat}. Slike drugog formata su automatski ignorisane.`);

    let kombinovaneSlike = [...files, ...filtriraneSlike];

    if (kombinovaneSlike.length > 10) {
      alert("Maksimalan broj slika za ovaj Endžin je 10. Višak je ignorisan.");
      kombinovaneSlike = kombinovaneSlike.slice(0, 10);
    }
  ...
  const getBackendRoute = () => {
    if (isVIP && !isAdmin) return BACKEND_ROUTES.PAID_16MP;
    return BACKEND_ROUTES.TRIAL_16MP;
  };
  ...
  const handleUpscaleAndDownload = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        let serverMessage = "Serverska greška prilikom generisanja ZIP-a.";
        try {
          const errData = await response.json();
          serverMessage = errData?.error || errData?.details || serverMessage;
        } catch {
          try {
      ...
      if (!blob || blob.size === 0) {
        throw new Error("Server je vratio prazan ZIP fajl.");
      }
      ...
      if (auth.currentUser && !isAdmin) {
        ...
        if (isVIP) {
          const docRef = doc(db, "vip_users", userEmail);
          if (novaKolicina <= 0) {
            await updateDoc(docRef, { optimizerCredits: 0 });
          } else {
            await updateDoc(docRef, { optimizerCredits: increment(-files.length) });
          }
        } else if (isTrial) {
          const docRef = doc(db, "v8_users", auth.currentUser.uid);
          if (novaKolicina <= 0) {
            await updateDoc(docRef, { credits_16mp: 0 });
          } else {
            await updateDoc(docRef, { credits_16mp: increment(-files.length) });
          }
        }
      }
    } catch (error) {
      console.error("[V8 16MP FRONTEND ERROR]", error);
      const message = error?.message || "Batch Upscale processing failed. Check server logs.";
      ...
      return (
        <div className="w-full max-w-5xl mx-auto mt-16 px-4 mb-16">
           <div className="bg-gradient-to-r from-[#1a0b2e] to-[#050505] border border-purple-500/40 rounded-[2.5rem] p-12 text-center shadow-[0_0_50px_rgba(168,85,247,0.15)] relative overflow-hidden">
              ...
              <h2 className="text-3xl md:text-4xl font-black text-white uppercase tracking-widest mb-4">ENTERPRISE TIER <span className="text-purple-500">UNLOCKED</span></h2>
              <p className="text-purple-200/60 font-bold uppercase tracking-widest text-[11px] md:text-sm max-w-2xl mx-auto">You possess the highest level V8 License. All protocols are fully operational at maximum capacity.</p>
           </div>
        </div>
      );
    }
    ...
    return (
      <div className="w-full max-w-5xl mx-auto mt-16 px-4 mb-16">
        <div className="text-center mb-12">
          <h2 className="text-4xl font-black text-white uppercase tracking-widest mb-4">{amountPaid > 0 ? "UPGRADE YOUR ACCESS." : "LIFETIME ACCESS."} <span className="text-orange-500 block md:inline mt-2 md:mt-0">CHOOSE YOUR V8 PLAN.</span></h2>
          <div className="mt-8 bg-[#0a0a0a]/90 border border-white/10 rounded-2xl p-8 text-left space-y-4 shadow-inner max-w-4xl mx-auto mb-8">
             <h4 className="text-orange-500 font-black uppercase tracking-[0.2em] text-[13px] border-b border-orange-500/20 pb-3 mb-4 flex items-center gap-2"><ShieldCheck className="w-5 h-5" /> V8 LICENSE PROTOCOL</h4>
        ...
        <div className="flex flex-wrap justify-center gap-6 w-full z-10 relative">
          ...
          {amountPaid < 150 && (
            <div className="w-full md:w-[calc(33.333%-1rem)] max-w-sm bg-[#050505] border border-blue-500/30 rounded-[2rem] p-8 flex flex-col hover:border-blue-500/60 transition-all shadow-xl">
                ...
                <button onClick={() => pokreniKupovinu('STARTER', 150)} className="w-full bg-zinc-800 text-white hover:bg-blue-500 py-4 rounded-xl font-black uppercase tracking-widest text-[13px] transition-all shadow-md">SELECT STARTER</button>
            </div>
          )}
          ...
          {amountPaid < 250 && (
            <div className="w-full md:w-[calc(33.333%-1rem)] max-w-sm bg-[#050505] border-2 border-orange-500/50 rounded-[2rem] p-8 flex flex-col relative hover:border-orange-500/80 transition-all shadow-[0_0_30px_rgba(234,88,12,0.15)] transform md:scale-105 z-10">
                ...
                <span className="text-4xl font-black text-orange-500 my-4 text-center flex items-center justify-center gap-3">{amountPaid > 0 ? `$${250 - amountPaid}` : "$250"}</span>
          ...
          {amountPaid < 550 && (
            <div className="w-full md:w-[calc(33.333%-1rem)] max-w-sm bg-[#050505] border border-purple-500/30 rounded-[2rem] p-8 flex flex-col hover:border-purple-500/60 transition-all shadow-xl">
                ...
                <span className="text-4xl font-black text-purple-400 my-4 text-center flex items-center justify-center gap-3">{amountPaid > 0 ? `$${550 - amountPaid}` : "$550"}</span>
                <div className="w-full text-left space-y-3 mb-8 text-[11px] text-zinc-400 font-bold uppercase tracking-widest flex-grow">
        ...
        {amountPaid > 0 && amountPaid < 550 && (
           <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-4xl mx-auto mt-12 bg-gradient-to-r from-[#190a05]/90 to-[#020617]/90 border border-orange-500/40 p-6 md:p-8 rounded-[2rem] flex flex-col md:flex-row items-center justify-center gap-8 shadow-[0_0_40px_rgba(234,88,12,0.25)] relative overflow-hidden backdrop-blur-md">
             ...
             <div className="w-16 h-16 bg-orange-950/50 rounded-full flex items-center justify-center border border-orange-500/50 relative flex-shrink-0 shadow-[0_0_20px_rgba(234,88,12,0.3)]"><div className="absolute inset-0 rounded-full border-t-2 border-orange-400 animate-spin"></div><ArrowUpCircle className="w-8 h-8 text-orange-400" /></div>
             <div className="text-center md:text-left relative z-10">
                <div className="inline-block bg-orange-900/30 border border-orange-500/30 px-3 py-1 rounded-full text-orange-300 font-bold uppercase tracking-widest text-[9px] mb-3">SMART UPGRADE SYSTEM ACTIVE</div>
                <h3 className="text-white text-lg md:text-xl font-black uppercase tracking-widest mb-2 drop-shadow-md">PRORATED UPGRADE POLICY</h3>
                <p className="text-zinc-300 text-[13px] md:text-[14px] leading-relaxed max-w-2xl font-medium">System radar has detected an active V8 License valued at <strong className="text-orange-400">${amountPaid}</strong> linked to your account. You will <strong className="text-white border-b border-orange-500/50 pb-0.5">only pay the exact difference</strong> to upgrade to a higher tier.</p>
             </div>
           </motion.div>
        )}
      </div>
              ...
              return (
                <div key={i} onClick={() => setOtvorenOpis(isOpen ? null : i)} className={`bg-white/5 border p-6 rounded-2xl transition-all duration-500 cursor-pointer relative overflow-hidden group ${isOpen ? 'border-orange-500/50 shadow-[0_0_15px_rgba(249,115,22,0.1)]' : 'border-white/5 hover:border-white/20'}`}>
                  <div className="relative z-10 flex justify-between items-center">
                    <div>
                      <h4 className={`text-[13px] md:text-[15px] font-black uppercase transition-colors duration-300 flex items-center gap-3 mb-2 ${isOpen ? 'text-orange-400' : 'text-blue-400'}`}><span className={`text-lg transition-colors duration-300 ${isOpen ? 'text-orange-500' : 'text-blue-600/60'}`}>💎</span> {item.t}</h4>
                  ...
                  <div className={`grid transition-all duration-500 ease-in-out relative z-10 ${isOpen ? 'grid-rows-[1fr] mt-4 opacity-100' : 'grid-rows-[0fr] opacity-0'}`}>
                    <div className="overflow-hidden"><div className="pt-4 border-t border-white/10"><p className="text-[11px] md:text-[12px] text-zinc-300 font-mono leading-relaxed border-l-2 border-orange-500 pl-3"><span className="text-orange-400 font-bold">Tech Insight:</span> {item.insight}</p></div></div>
                  </div>
  ...
  return (
    <div className="bg-[#050505] p-8 md:p-12 rounded-[2.5rem] border border-[#FF8C00]/30 shadow-[0_0_50px_rgba(255,140,0,0.1)] max-w-6xl mx-auto mt-28 relative">
      ...
      <AnimatePresence>
        {isCheckoutOpen && (
          <V8SecureCheckout isOpen={isCheckoutOpen} onClose={() => setIsCheckoutOpen(false)} productName={checkoutProduct} price={checkoutPrice} />
        )}
      </AnimatePresence>

      {/* 🔥 ADMIN BROJAČ SA BOJAMA KOJE PRATE TAB (Narandžasta) 🔥 */}
            ...
            <div className="bg-black/50 border border-white/10 rounded-2xl p-4 flex flex-col items-center justify-center min-h-[300px] relative overflow-hidden group">
               ...
               {showcase.before ? (
                 <><img src={showcase.before} alt="Before Upscale" className="w-full h-full object-contain relative z-10 cursor-pointer hover:scale-[1.02] transition-transform duration-500" onClick={() => setFullScreenImageUrl(showcase.before)} />
                   {isAdmin && (<button onClick={(e) => deleteShowcaseImage(e, 'before')} className="absolute top-4 right-4 bg-red-600 hover:bg-red-500 text-white p-2 rounded-full z-30 shadow-lg opacity-0 group-hover:opacity-100 transition-opacity"><Trash2 size={16} /></button>)}
                   <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center pointer-events-none z-20"><Eye className="w-12 h-12 text-white/50" /></div>
                 </>
               ) : ( isAdmin ? ( <div className="flex flex-col items-center justify-center cursor-pointer opacity-50 hover:opacity-100 transition-opacity" onClick={() => beforeImgRef.current.click()}><Upload className="w-10 h-10 text-zinc-500 mb-2" /><span className="text-[11px] font-bold text-zinc-400 uppercase tracking-widest">{isUploadingShowcase.before ? "UPLOADING..." : "UPLOAD BEFORE IMAGE"}</span><input type="file" ref={beforeImgRef} accept="image/*" className="hidden" onChange={(e) => handleShowcaseUpload(e, 'before')} /></div> ) : ( <span className="text-zinc-600 text-sm font-bold uppercase tracking-widest">Image Unavailable</span> ) )}
            </div>
            ...
            <div className="bg-black/50 border border-orange-500/30 rounded-2xl p-4 flex flex-col items-center justify-center min-h-[300px] relative overflow-hidden group shadow-[0_0_20px_rgba(255,140,0,0.05)]">
               ...
               {showcase.after ? (
                 <><img src={showcase.after} alt="After Upscale" className="w-full h-full object-contain relative z-10 cursor-pointer hover:scale-[1.02] transition-transform duration-500" onClick={() => setFullScreenImageUrl(showcase.after)} />
                   {isAdmin && (<button onClick={(e) => deleteShowcaseImage(e, 'after')} className="absolute top-4 right-4 bg-red-600 hover:bg-red-500 text-white p-2 rounded-full z-30 shadow-lg opacity-0 group-hover:opacity-100 transition-opacity"><Trash2 size={16} /></button>)}
                   <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center pointer-events-none z-20"><Eye className="w-12 h-12 text-white/50" /></div>
                 </>
               ) : ( isAdmin ? ( <div className="flex flex-col items-center justify-center cursor-pointer opacity-50 hover:opacity-100 transition-opacity" onClick={() => afterImgRef.current.click()}><Upload className="w-10 h-10 text-orange-500/50 mb-2" /><span className="text-[11px] font-bold text-orange-500/50 uppercase tracking-widest">{isUploadingShowcase.after ? "UPLOADING..." : "UPLOAD AFTER IMAGE"}</span><input type="file" ref={afterImgRef} accept="image/*" className="hidden" onChange={(e) => handleShowcaseUpload(e, 'after')} /></div> ) : ( <span className="text-zinc-600 text-sm font-bold uppercase tracking-widest">Image Unavailable</span> ) )}
            </div>
         </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 relative z-10 mb-16 items-stretch">
          ...
          <div className="flex flex-col gap-6 h-full">
             ...
             <div className={`relative border-2 border-dashed rounded-2xl p-6 flex-1 flex flex-col items-center justify-center text-center transition-all min-h-[320px] ${dragActive ? 'border-[#FF8C00] bg-[#FF8C00]/10' : 'border-white/20 bg-black/50 hover:border-[#FF8C00]/50'} ${files.length > 0 ? 'border-solid border-[#FF8C00]/50' : ''}`} onDragEnter={handleDrag} onDragLeave={handleDrag} onDragOver={handleDrag} onDrop={handleDrop}>
                ...
                {files.length > 0 ? (
                  <div className="relative w-full h-full flex flex-col justify-center items-center bg-[#050505] group rounded-xl overflow-hidden p-6 border border-orange-500/30">
                    <div className="text-center mb-6"><Layers className="w-16 h-16 text-orange-500 mb-4 mx-auto animate-pulse" /><span className="text-2xl font-black text-white">{files.length} / 10 IMAGES BATCHED</span></div>
                    <div className="flex gap-4 relative z-20">
                        {files.length < 10 && (<button type="button" onClick={(e) => { e.stopPropagation(); inputRef.current.click(); }} className="bg-zinc-800 text-white px-6 py-3 rounded-full text-xs font-black uppercase hover:bg-zinc-700 transition-all shadow-lg border border-zinc-600">+ ADD MORE</button>)}
                        <button type="button" onClick={obrisiSlike} className="bg-red-600/90 text-white px-6 py-3 rounded-full text-xs font-black uppercase hover:bg-red-500 transition-all shadow-[0_0_20px_rgba(220,38,38,0.4)]">CLEAR BATCH</button>
                    </div>
                  </div>
                ) : ( <div className="flex flex-col items-center gap-3 cursor-pointer relative z-10" onClick={() => inputRef.current.click()}><div className="bg-white/5 p-4 rounded-full"><Upload className="w-8 h-8 text-zinc-400" /></div><div><p className="text-white font-bold text-sm">Drag & Drop up to 10 images</p><p className="text-zinc-500 text-xs mt-1">or click to browse files</p></div></div> )}
             </div>
          </div>
                 ...
                 <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                    {v8Logs.slice(0, activeLog).map((log, index) => (
                       <div key={index} className="mb-2">
                          {log.includes('🚀') ? <span className="text-orange-500 font-black">{log}</span> : log.includes('💎') ? <span className="text-blue-400">{log}</span> : log.includes('✅') ? <span className="text-emerald-400 font-black">{log}</span> : <span>{log}</span>}
                       </div>
                    ))}
                 </motion.div>
               ) : ( <div className="absolute inset-0 flex flex-col items-center justify-center opacity-10"><Cpu className="w-16 h-16 mb-4 text-zinc-400" /><span className="font-black text-[10px] tracking-widest uppercase">AWAITING BATCH INIT</span></div> )}
             </div>
             ...
             <div className="mt-auto pt-2 flex flex-col gap-4">
               <button 
                  ...
                  className={`w-full font-black text-[14px] uppercase tracking-widest py-4 rounded-xl transition-all flex items-center justify-center gap-3 disabled:opacity-50
                    ${downloadStatus === 'success' ? 'bg-emerald-600 hover:bg-emerald-500 text-white shadow-[0_0_30px_rgba(16,185,129,0.4)] hover:scale-[1.02]' 
                      : (credits <= 0 && !isAdmin) ? 'bg-red-900/50 text-red-500 border border-red-500/50 cursor-not-allowed' 
                ...
                >
                 ...
                 {isProcessing ? "PROCESSING V9 ENGINE..." : downloadStatus === 'success' ? "DOWNLOAD 16MP BATCH (ZIP)" : (credits <= 0 && !isAdmin) ? "INSUFFICIENT CREDITS" : `INITIATE 16MP BATCH UPSCALE${(!isVIP && !isAdmin) ? ' (TRIAL)' : ''}`}
               </button>
             </div>
          </div>
        </div>
