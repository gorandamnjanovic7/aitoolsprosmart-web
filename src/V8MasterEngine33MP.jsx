...
const FullScreenLightbox = ({ imageUrl, onClose }) => {
  ...
  return createPortal(
      <div className="fixed inset-0 z-[999999] bg-[#0f172a]/95 flex items-center justify-center p-4" onClick={onClose}>
          <button type="button" onClick={(e) => { e.stopPropagation(); onClose(); }} className="absolute top-6 right-6 md:top-10 md:right-10 bg-yellow-500 text-black p-4 rounded-full font-black z-[1000000] shadow-[0_0_20px_rgba(234,179,8,0.5)]"><X size={32} strokeWidth={3} /></button>
          <img src={imageUrl} alt="Full Screen Preview" className="max-w-full max-h-[90vh] object-contain rounded-2xl shadow-[0_0_80px_rgba(234,179,8,0.4)] border border-yellow-500/30 relative z-[999999]" onClick={(e) => e.stopPropagation()} />
      </div>, document.body
  );
};

const V8MasterEngine33MP = () => {
  ...
  const v8Logs = [
    "🚀 VISIONARY FACTORY V8 | IGNITING 33.2MP MASTER ENGINE...",
    "💎 1. Extreme 33.2MP Upscale (LANCZOS) initiated",
    "💎 2. Texture-safe Contributor Quality Cleanup",
    "💎 3. Premium Sharpness 33.2MP Tuned",
    ...
    "💎 10. Compiling ZIP package with TXT report",
  ...
  const prepareCheckoutPackage = (paketName, fullPrice) => {
    ...
    const naslovCheckouta = isUpgrade
      ? `MASTER 33MP - ${paketName.toUpperCase()} (UPGRADE)`
      : `MASTER 33MP - ${paketName.toUpperCase()}`;

    setCheckoutProduct(naslovCheckouta);
    setCheckoutPrice(finalPrice);

    return {
      productName: naslovCheckouta,
      price: finalPrice
    };
  };
      ...
      unsubTrial = onSnapshot(doc(db, "v8_users", user.uid), snap => {
          if (snap.exists()) {
             ...
             if (userData.credits_33mp > 0) {
                 setIsTrial(true);
                 if(!adminCheck) setCredits(userData.credits_33mp);
             } else {
                 setIsTrial(false);
  ...
  const handleShowcaseUpload = async (e, type) => {
    ...
    try {
      ...
      await setDoc(doc(db, "v8_settings", "showcase_33mp"), { [type]: resData.secure_url }, { merge: true });
  ...
  const deleteShowcaseImage = async (e, type) => {
    e.stopPropagation();
    if(window.confirm("Obrisati ovu sliku?")) await setDoc(doc(db, "v8_settings", "showcase_33mp"), { [type]: '' }, { merge: true });
  };
  ...
  const processSelectedFiles = async (selectedFiles) => {
    ...
    if (combined.length > MAX_FILES) {
        alert(`Maskimalno ${MAX_FILES} slika po batch-u.`);
  ...
  const handleUpscaleAndDownload = async () => {
    ...
    try {
      ...
      const backendRoute = (isVIP && !isAdmin) ? '/api/v8-33mp-process' : '/api/v8-33mp-trial-process';
      ...
      if (!response.ok) {
          ...
          throw new Error(errorData.error || "Server Error during 33MP processing.");
      }
      ...
      if (auth.currentUser && !isAdmin) {
          ...
          if (isVIP) {
              ...
              if (novaKolicina <= 0) await setDoc(docRef, { ['MASTER33_credits']: 0, ['MASTER33_cooldown']: serverTimestamp() }, { merge: true });
              else await setDoc(docRef, { ['MASTER33_credits']: increment(-files.length) }, { merge: true });
          } else if (isTrial) {
              const uid = auth.currentUser.uid;
              const docRef = doc(db, "v8_users", uid);
              if (novaKolicina <= 0) await setDoc(docRef, { ['credits_33mp']: 0 }, { merge: true });
              else await setDoc(docRef, { ['credits_33mp']: increment(-files.length) }, { merge: true });
          }
      }
    } catch (error) {
    ...
    if (amountPaid >= 550) {
      return (
        <div className="w-full max-w-5xl mx-auto mt-16 px-4">
           <div className="bg-gradient-to-r from-[#1a180b] to-[#050505] border border-yellow-500/40 rounded-[2.5rem] p-12 text-center shadow-[0_0_50px_rgba(234,179,8,0.15)] relative overflow-hidden">
              <Crown className="w-20 h-20 text-yellow-500 mx-auto mb-6 drop-shadow-[0_0_20px_rgba(234,179,8,0.6)]" />
              <h2 className="text-3xl md:text-4xl font-black text-white uppercase tracking-widest mb-4">
                ENTERPRISE TIER <span className="text-yellow-500">UNLOCKED</span>
              </h2>
              <p className="text-yellow-200/60 font-bold uppercase tracking-widest text-[11px] md:text-sm max-w-2xl mx-auto">
                You possess the highest level V8 License. The 33.2MP Master Engine is fully operational.
              </p>
           </div>
        </div>
      );
      ...
      <div className="w-full max-w-5xl mx-auto mt-16 px-4">
        <div className="text-center mb-12">
          <h2 className="text-4xl font-black text-white uppercase tracking-widest mb-4">
            {amountPaid > 0 ? "UPGRADE YOUR ACCESS." : "LIFETIME ACCESS."} <span className="text-yellow-500 block md:inline mt-2 md:mt-0">CHOOSE YOUR V8 PLAN.</span>
          </h2>
          ...
          <div className="mt-8 bg-[#0a0a0a]/90 border border-white/10 rounded-2xl p-8 text-left space-y-4 shadow-inner max-w-4xl mx-auto mb-8">
             <h4 className="text-yellow-500 font-black uppercase tracking-[0.2em] text-[13px] border-b border-yellow-500/20 pb-3 mb-4 flex items-center gap-2">
        ...
        <div className="flex flex-wrap justify-center gap-6 w-full z-10 relative">
          ...
          {amountPaid < 250 && (
            <div className={`w-full md:w-[calc(33.333%-1rem)] max-w-sm bg-[#050505] border-2 rounded-[2rem] p-8 flex flex-col relative transition-all transform md:scale-105 z-10 border-yellow-500/50 hover:border-yellow-500/80 shadow-[0_0_30px_rgba(234,179,8,0.15)]`}>
                <div className={`absolute top-0 left-0 w-full h-2 rounded-t-[1.9rem] bg-gradient-to-r from-yellow-600 to-amber-400`}></div>
                <div className={`absolute -top-4 left-1/2 -translate-x-1/2 text-black px-6 py-1.5 rounded-full font-black text-[10px] uppercase tracking-widest shadow-lg bg-yellow-500`}>Bestseller</div>
                
                <div className={`w-12 h-12 flex items-center justify-center rounded-full mb-6 mx-auto mt-2 bg-yellow-500/10`}>
                   <Zap className={`w-6 h-6 text-yellow-500`} />
                </div>
                <h3 className="text-xl font-black text-white uppercase text-center">Pro</h3>
                <span className={`text-4xl font-black my-4 text-center flex items-center justify-center gap-3 text-yellow-500`}>
        ...
        {amountPaid > 0 && amountPaid < 550 && (
           <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-4xl mx-auto mt-12 mb-10 bg-gradient-to-r from-[#1a180b]/90 to-[#020617]/90 border border-yellow-500/40 p-6 md:p-8 rounded-[2rem] flex flex-col md:flex-row items-center justify-center gap-8 shadow-[0_0_40px_rgba(234,179,8,0.2)] relative overflow-hidden backdrop-blur-md">
             ...
             <div className="absolute top-0 left-1/2 -translate-x-1/2 w-3/4 h-1 bg-gradient-to-r from-transparent via-yellow-500 to-transparent opacity-50"></div>
             
             <div className="w-16 h-16 bg-yellow-950/50 rounded-full flex items-center justify-center border border-yellow-500/50 relative flex-shrink-0 shadow-[0_0_20px_rgba(234,179,8,0.3)]">
             ...
             <div className="text-center md:text-left relative z-10">
                <div className="inline-block bg-yellow-900/30 border border-yellow-500/30 px-3 py-1 rounded-full text-yellow-300 font-bold uppercase tracking-widest text-[9px] mb-3">
                ...
                <p className="text-zinc-300 text-[13px] md:text-[14px] leading-relaxed max-w-2xl font-medium">
                  System radar has detected an active V8 License valued at <strong className="text-yellow-400">${amountPaid}</strong> linked to your account. You will <strong className="text-white border-b border-yellow-500/50 pb-0.5">only pay the exact difference</strong> to upgrade to a higher tier.
                </p>
             </div>
           </motion.div>
        )}
  ...
  const renderV8Manifest = () => {
    const specifikacije = [
        { t: "1. 33.2MP Upscale", d: "Extreme Lanczos Interpolation.", insight: "Pushes native resolution exactly to marketplace maximums (e.g. 7680x4320) without distorting base structure." },
        ...
        { t: "3. Premium Sharpness", d: "UnsharpMask calibrated for 33MP.", insight: "Restores crisp edge clarity post-upscale, eliminating the soft 'upscaled' blur without creating amateur halos." },
    ...
    return (
        <div className="w-full max-w-5xl mx-auto mb-16 bg-black/40 border border-white/5 rounded-[2rem] p-8 md:p-10 relative z-10 shadow-[0_20px_60px_rgba(0,0,0,0.6)]">
          <div className="text-center mb-10">
            ...
            <p className="text-[12px] md:text-[14px] text-yellow-500 font-bold uppercase tracking-[0.3em] mt-3 italic">Technical Specifications 33.2MP</p>
          </div>
              ...
              return (
                <div 
                  ...
                  className={`bg-white/5 border p-6 rounded-2xl transition-all duration-500 cursor-pointer relative overflow-hidden group ${
                    isOpen ? 'border-yellow-500/50 shadow-[0_0_15px_rgba(234,179,8,0.1)]' : 'border-white/5 hover:border-white/20'
                  }`}
                >
                  <div className="relative z-10 flex justify-between items-center">
                    <div>
                      <h4 className={`text-[13px] md:text-[15px] font-black uppercase transition-colors duration-300 flex items-center gap-3 mb-2 ${isOpen ? 'text-yellow-500' : 'text-zinc-400'}`}>
                        <span className={`text-lg transition-colors duration-300 ${isOpen ? 'text-yellow-500' : 'text-zinc-600'}`}>💎</span> 
                        {item.t}
                      </h4>
                    ...
                    <div className={`ml-4 text-xs md:text-sm font-black transition-all duration-500 ${isOpen ? 'rotate-180 text-yellow-500 drop-shadow-[0_0_8px_rgba(234,179,8,0.8)]' : 'text-zinc-500 group-hover:text-zinc-400'}`}>
                      ▼
                    </div>
                  </div>
                  <div className={`grid transition-all duration-500 ease-in-out relative z-10 ${isOpen ? 'grid-rows-[1fr] mt-4 opacity-100' : 'grid-rows-[0fr] opacity-0'}`}>
                    <div className="overflow-hidden">
                      <div className="pt-4 border-t border-white/10">
                        <p className="text-[11px] md:text-[12px] text-zinc-300 font-mono leading-relaxed border-l-2 border-yellow-500 pl-3">
                          <span className="text-yellow-500 font-bold">Tech Insight:</span> {item.insight}
                        </p>
                      </div>
                    </div>
                  </div>
                  <div className={`absolute inset-0 bg-gradient-to-br from-yellow-500/5 to-transparent transition-opacity duration-500 ${isOpen ? 'opacity-100' : 'opacity-0'}`}></div>
                </div>
              );
            })}
          </div>
  ...
  return (
    <div className="bg-[#050505] p-8 md:p-12 rounded-[2.5rem] border border-[#FF8C00]/30 shadow-[0_0_50px_rgba(234,179,8,0.1)] max-w-6xl mx-auto mt-28 relative overflow-hidden">
      ...
      <LoginRequiredModal
        ...
        packageName={checkoutProduct || "MASTER 33MP PACKAGE"}
      ...
      {(isVIP || isTrial) && (
        <div className="absolute top-6 left-1/2 -translate-x-1/2 z-50">
           <motion.div 
              ...
              className="bg-black/80 backdrop-blur-xl border border-yellow-500/50 px-6 py-2 rounded-full shadow-[0_0_20px_rgba(234,179,8,0.3)] flex items-center gap-4"
           >
              <Cpu className="w-4 h-4 text-yellow-500 animate-pulse" />
              <div className="flex flex-col items-center">
                 <span className="text-[9px] uppercase tracking-[0.2em] font-black text-zinc-400 leading-none">33MP {(!isVIP || isAdmin) ? 'TRIAL ' : ''}CREDITS</span>
      ...
      <motion.div 
          ...
          className={`relative w-full mx-auto mb-12 rounded-[2.5rem] overflow-hidden border border-white/10 shadow-[0_0_50px_rgba(234,179,8,0.15)]`}
      >
          <video autoPlay loop muted playsInline className="absolute inset-0 w-full h-full object-cover opacity-50 z-0 pointer-events-none">
             <source src="/master33-bg.mp4" type="video/mp4" />
          </video>
          ...
          <div className="relative z-10 py-16 px-6 text-center flex flex-col items-center">
              <div className="inline-block bg-yellow-600/10 border border-yellow-500/30 px-5 py-2 rounded-full text-yellow-500 font-black uppercase tracking-[0.3em] text-[10px] mb-6 animate-pulse shadow-[0_0_20px_rgba(234,179,8,0.2)] backdrop-blur-sm">
              ...
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-black italic uppercase tracking-tighter text-white mb-6 drop-shadow-[0_10px_30px_rgba(0,0,0,0.9)] flex items-center justify-center gap-4 flex-wrap">
                <Cpu className="text-yellow-500 w-12 h-12 md:w-16 md:h-16 drop-shadow-[0_0_15px_rgba(234,179,8,0.8)]" />
                MASTER ENGINE 
                <span className="text-xl md:text-3xl font-black not-italic tracking-widest ml-2 px-4 py-1 rounded-full flex items-center border text-[#eab308] drop-shadow-[0_0_15px_rgba(234,179,8,0.6)] border-[#eab308]/30 bg-[#eab308]/10">
                   33.2MP
                </span>
              </h1>
              
              <div className="bg-[#050505]/80 backdrop-blur-md border border-yellow-500/20 p-8 rounded-[2rem] max-w-4xl mx-auto text-left shadow-2xl mb-8">
                <h4 className="text-white font-black uppercase tracking-widest text-sm mb-4 border-b border-white/10 pb-4">The Master Protocol:</h4>
                <p className="text-zinc-300 text-[13px] leading-relaxed mb-4">The 33.2MP Master Engine pushes your raw files to absolute marketplace limits. It strictly applies <strong>Lanczos Interpolation</strong>, followed by surgical noise reduction that targets AI compression debris while fully preserving high-frequency details.</p>
      ...
      <div className="w-full max-w-5xl mx-auto mb-16 relative z-10">
         <div className="text-center mb-8">
            ...
            <p className="text-[12px] text-zinc-400 font-bold uppercase tracking-widest mt-2">Before & After 33.2MP Master Enhancement</p>
         </div>
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-black/50 border border-yellow-500/30 rounded-2xl p-4 flex flex-col items-center justify-center min-h-[300px] relative overflow-hidden group shadow-[0_0_20px_rgba(234,179,8,0.05)]">
               <span className="absolute top-4 left-4 bg-yellow-500 text-black text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-widest z-20 shadow-md">After (Master 33MP)</span>
               ...
               {showcase.after ? (
                 ...
                 isAdmin ? (
                   <div className="flex flex-col items-center justify-center cursor-pointer opacity-50 hover:opacity-100 transition-opacity" onClick={() => afterImgRef.current.click()}>
                      <Upload className="w-10 h-10 text-yellow-500/50 mb-2" />
                      <span className="text-[11px] font-bold text-yellow-500/50 uppercase tracking-widest">{isUploadingShowcase.after ? "UPLOADING..." : "UPLOAD AFTER IMAGE"}</span>
      ...
      <div className={`transition-all duration-500 ${(!isVIP && !isTrial && !isAdmin) ? 'opacity-30 grayscale-[70%] pointer-events-none' : ''}`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 relative z-10 mb-16 items-stretch">
          ...
          <div className="flex flex-col gap-6 h-full">
             <label className="text-yellow-500 font-black text-[11px] tracking-widest uppercase flex items-center gap-2">
               <Layers size={14} /> 1. BATCH UPLOAD (UP TO {MAX_FILES} IMAGES)
             </label>
             <div className={`relative border-2 border-dashed rounded-2xl p-6 flex-1 flex flex-col items-center justify-center text-center transition-all min-h-[320px] ${dragActive ? 'border-yellow-500 bg-yellow-500/10' : 'border-white/20 bg-black/50 hover:border-yellow-500/50'} ${files.length > 0 ? 'border-solid border-yellow-500/50' : ''}`} onDragEnter={handleDrag} onDragLeave={handleDrag} onDragOver={handleDrag} onDrop={handleDrop}>
                <input ref={inputRef} type="file" accept="image/*" multiple onChange={handleChange} className="hidden" />
                
                {files.length > 0 ? (
                  <div className="relative w-full h-full flex flex-col justify-center items-center bg-[#050505] group rounded-xl overflow-hidden p-6 border border-yellow-500/30">
                    <div className="text-center mb-6">
                       <Layers className="w-16 h-16 text-yellow-500 mb-4 mx-auto animate-pulse" />
                 ...
                 <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                    {v8Logs.slice(0, activeLog).map((log, index) => (
                       <div key={index} className="mb-2">
                          {log.includes('🚀') ? <span className="text-yellow-500 font-black">{log}</span> : 
               ...
               <button 
                  ...
                  className={`w-full font-black text-[14px] uppercase tracking-widest py-4 rounded-xl transition-all flex items-center justify-center gap-3 disabled:opacity-50
                    ${downloadStatus === 'success' 
                      ...
                      : 'bg-gradient-to-r from-yellow-600 to-amber-500 hover:from-yellow-500 hover:to-amber-400 text-black shadow-[0_0_30px_rgba(234,179,8,0.4)] hover:scale-[1.02]'
                    }`}
                >
                 ...
                 {isProcessing ? "PROCESSING MASTER ENGINE..." : downloadStatus === 'success' ? "DOWNLOAD 33MP BATCH (ZIP)" : (credits <= 0 && !isAdmin) ? "INSUFFICIENT CREDITS" : `INITIATE 33MP BATCH UPSCALE${(!isVIP && !isAdmin) ? ' (TRIAL)' : ''}`}
               </button>
             </div>
          </div>
...
export default V8MasterEngine33MP;
// KRAJ FAJLA: V8MasterEngine33MP.jsx