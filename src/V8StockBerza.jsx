...
const V8StockBerza = () => {
  ...
  // 👇 V8 AUTOMATSKI OPIS 👇
  useEffect(() => {
    if (noviFormat === '16:9 (20 SLIKA)' || noviFormat === '16:9 (20 IMAGES)') {
    ...
    } else if (noviFormat === 'SVI FORMATI (80 SLIKA)' || noviFormat === 'ALL FORMATS (80 IMAGES)') {
...
// 👇 V8 AUTOMATSKI PREVOD KATEGORIJE 👇
  ...
  // --- V8 TURBO MAGIC SCRIPT ZA ČIŠĆENJE BAZE ---
  const v8TurboCleanDB = async () => {
      if (!window.confirm("V8 UPOZORENJE: Ovo će automatski prevesti sve stare formate i dodati opis na engleskom (bez diranja RSD cena za IPS). Da li ste sigurni?")) return;
      ...
      try {
          ...
          for (const docSnap of snap.docs) {
              ...
              // 1. Prebaci srpske nazive i kategorije u nove varijable (ako su prazne)
              ...
              // 3. Ubaci engleski opis tamo gde fali
              if (!data.opisEn || data.opisEn.trim() === "") {
                  if (newFormat === '16:9 (20 IMAGES)' || newFormat === '16:9 (20 SLIKA)') {
                      updates.opisEn = "PACKAGE CONTENTS: 20 PREMIUM AI VISUALS IN ULTRA-WIDE 16:9. PERFECT FOR WEBSITES AND YT. VALUE OVER €250.";
                      needsUpdate = true;
                  } else if (newFormat === 'ALL FORMATS (80 IMAGES)' || newFormat === 'SVI FORMATI (80 SLIKA)') {
          ...
          alert(`V8 Turbo Clean završen! ${count} paketa je ažurirano.`);
          fetchPaketi(); 
      } catch (error) {
          console.error("Clean error: ", error);
          alert("Greška tokom čišćenja baze: " + error.message);
      }
  };
  // ---------------------------------------------
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                      ...
                      <div className="flex flex-col gap-2">
                          ...
                          <div className="flex flex-col sm:flex-row gap-2">
                              <label className={`cursor-pointer flex-1 p-3 rounded-xl border-2 transition-all text-center font-black text-[10px] uppercase ${noviFormat === '16:9 (20 SLIKA)' || noviFormat === '16:9 (20 IMAGES)' ? 'bg-[#FF8C00]/20 border-[#FF8C00] text-[#FF8C00]' : 'bg-black border-white/10 text-zinc-500'}`}>
                                  <input type="radio" name="format" value="16:9 (20 IMAGES)" checked={noviFormat === '16:9 (20 SLIKA)' || noviFormat === '16:9 (20 IMAGES)'} onChange={(e) => setNoviFormat(e.target.value)} className="hidden" />
                                  16:9 (20 slika)
                              </label>
                              <label className={`cursor-pointer flex-1 p-3 rounded-xl border-2 transition-all text-center font-black text-[10px] uppercase ${noviFormat === 'SVI FORMATI (80 SLIKA)' || noviFormat === 'ALL FORMATS (80 IMAGES)' ? 'bg-[#FF8C00]/20 border-[#FF8C00] text-[#FF8C00]' : 'bg-black border-white/10 text-zinc-500'}`}>
                                  <input type="radio" name="format" value="ALL FORMATS (80 IMAGES)" checked={noviFormat === 'SVI FORMATI (80 SLIKA)' || noviFormat === 'ALL FORMATS (80 IMAGES)'} onChange={(e) => setNoviFormat(e.target.value)} className="hidden" />
                                  Svi formati
                              </label>
                          </div>
                      </div>
