// POČETAK FAJLA: V8JsonDeBrendingPage.jsx
...
const V8JsonDeBrendingExtractorPage = () => {
  ...
  const handleShowcaseUpload = async (e, type) => {
    ...
    try {
      ...
      await setDoc(doc(db, "v8_settings", "showcase_debranding"), { [type]: resData.secure_url }, { merge: true });
    } catch (err) { 
  ...
  const extractDNA = async () => {
    ...
    try {
        const response = await fetch(`${BASE_BACKEND_URL}/api/v8-debrand`, { method: 'POST', body: formData });
  ...
  const renderV8Manifest = () => {
    const specifikacije = [
        ...
        { t: "2. Explicit No-Brand Lock", d: "Forces clean, minimalist subject reconstruction.", insight: "Instructs the model to output a minimalist design stripped of all brand logos, preventing accidental trademark generation." },
        { t: "3. Lighting & Materials", d: "Preserves high-end material integrity and light behavior.", insight: "Captures carbon fiber, brushed metal, and god rays without copying the original brand's commercial markers." },
        { t: "4. Camera Equipment", d: "Recommends cinematic gear and specific lenses.", insight: "Uses professional camera language (e.g., ARRI Alexa 65, Leica Summilux) to improve depth and realism." },
        { t: "5. Backend-Ready Architecture", d: "Optimized for Node.js integrations.", insight: "Uses Base64 image encoding and direct HTTP API handling with flexible API key loading." },
        { t: "6. Atmosphere & Composition", d: "Captures spatial design and cinematic mood.", insight: "Describes camera angles, weather, motion blur, and foreground/background relationships for perfect recreation." },
        { t: "7. Chromatic Recalibration", d: "Maps and preserves the exact cinematic color grade.", insight: "Extracts underlying color temperatures, contrast ratios, and split-toning profiles to ensure the white-label output maintains its original luxury mood." },
        { t: "8. Spatial Depth Isolation", d: "Reconstructs precise focal planes and bokeh characteristics.", insight: "Translates depth-of-field metadata and micro-contrast, keeping the primary subject hyper-sharp while generating smooth, cinematic background separation." }
      ];
      ...
      return (
        <div className="w-full max-w-5xl mx-auto mb-16 bg-black/40 border border-white/5 rounded-[2rem] p-8 md:p-10 relative z-10 shadow-[0_20px_60px_rgba(0,0,0,0.6)]">
          <div className="text-center mb-10">
            <h2 className="text-3xl md:text-4xl font-black uppercase tracking-[0.2em] text-white">DE-BRANDING DNA ENGINE</h2>
            <p className="text-[12px] md:text-[14px] text-orange-400 font-bold uppercase tracking-[0.3em] mt-3 italic">Technical Specifications V2.0</p>
          </div>
                  ...
                  <div className={`grid transition-all duration-500 ease-in-out relative z-10 ${isOpen ? 'grid-rows-[1fr] mt-4 opacity-100' : 'grid-rows-[0fr] opacity-0'}`}>
                    <div className="overflow-hidden">
                      <div className="pt-4 border-t border-white/10">
                        <p className="text-[11px] md:text-[13px] text-zinc-300 font-mono leading-relaxed border-l-2 border-orange-500 pl-3">
    ...
    <div className="bg-[#050505] p-8 md:p-12 rounded-[2.5rem] border border-[#FF8C00]/30 shadow-[0_0_50px_rgba(255,140,0,0.1)] max-w-6xl mx-auto mt-28 relative">
      ...
      >
          ...
          <div className="relative z-10 py-16 px-6 text-center flex flex-col items-center">
              ...
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-black italic uppercase tracking-tighter text-white mb-6 drop-shadow-[0_10px_30px_rgba(0,0,0,0.9)] flex items-center justify-center gap-4 flex-wrap">
                  ...
                  JSON <span className="text-transparent bg-clip-text bg-gradient-to-b from-orange-500 to-amber-600 drop-shadow-none">DeBRENDING EXTRACTOR</span>
              </h1>
              ...
              <div className="bg-[#050505]/80 backdrop-blur-md border border-orange-500/20 p-8 rounded-[2rem] max-w-4xl mx-auto text-left shadow-2xl mb-8">
                  ...
                  <p className="text-zinc-300 text-[13px] leading-relaxed mb-4">
                      The V8 Cinematic Protocol acts as a <strong>forensic visual cleaner</strong>. It deconstructs your master image—identifying logos, watermarks, and brand identifiers—while preserving lighting vectors, color temperature, material textures, and structural composition, converting this data into a <strong>pure white-label JSON blueprint</strong>.
                  </p>
                  <p className="text-zinc-300 text-[13px] leading-relaxed">
                      Once extracted, this "DNA" becomes your legally safe master template. You can now force-apply this exact stylistic blueprint to any target AI generator without brand violations. The result is <strong>perfect aesthetic continuity</strong> across every platform with zero structure loss.
                  </p>
              </div>
      ...
      <div className="w-full max-w-5xl mx-auto mb-16 relative z-10">
         <div className="text-center mb-8">
            ...
            <p className="text-[12px] text-zinc-400 font-bold uppercase tracking-widest mt-2">Before & After De-Branding</p>
         </div>
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-black/50 border border-white/10 rounded-2xl p-4 flex flex-col items-center justify-center min-h-[300px] relative overflow-hidden group">
               <span className="absolute top-4 left-4 bg-zinc-800 text-white text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-widest z-20 shadow-md">Before (Branded)</span>
               
               {showcase.before ? (
                 <>
                   <img src={showcase.before} alt="Before DeBranding" className="w-full h-full object-contain relative z-10 cursor-pointer hover:scale-[1.02] transition-transform duration-500" onClick={() => setFullScreenImageUrl(showcase.before)} />
                   {isAdmin && (
            ...
            <div className="bg-black/50 border border-emerald-500/30 rounded-2xl p-4 flex flex-col items-center justify-center min-h-[300px] relative overflow-hidden group shadow-[0_0_20px_rgba(16,185,129,0.05)]">
               <span className="absolute top-4 left-4 bg-emerald-500 text-black text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-widest z-20 shadow-md">After (De-Branded)</span>
               
               {showcase.after ? (
                 <>
                   <img src={showcase.after} alt="After DeBranding" className="w-full h-full object-contain relative z-10 cursor-pointer hover:scale-[1.02] transition-transform duration-500" onClick={() => setFullScreenImageUrl(showcase.after)} />
                   {isAdmin && (
                 ...
                 isAdmin ? (
                   <div className="flex flex-col items-center justify-center cursor-pointer opacity-50 hover:opacity-100 transition-opacity" onClick={() => afterImgRef.current.click()}>
                      <Upload className="w-10 h-10 text-emerald-500/50 mb-2" />
                      <span className="text-[11px] font-bold text-emerald-500/50 uppercase tracking-widest">{isUploadingShowcase.after ? "UPLOADING..." : "UPLOAD AFTER IMAGE"}</span>
          ...
          <div className="flex flex-col gap-6 h-full">
             ...
             <div className={`relative border-2 border-dashed rounded-2xl p-6 flex-1 flex flex-col items-center justify-center text-center transition-all min-h-[320px] ${dragActive ? 'border-[#FF8C00] bg-[#FF8C00]/10' : 'border-white/20 bg-black/50 hover:border-[#FF8C00]/50'} ${previewUrl ? 'border-solid border-[#FF8C00]/50' : ''}`} onDragEnter={handleDrag} onDragLeave={handleDrag} onDragOver={handleDrag} onDrop={handleDrop}>
               ...
               {previewUrl ? (
                 ...
                 <div className="flex flex-col items-center gap-3 cursor-pointer" onClick={() => inputRef.current.click()}>
                   ...
                   <div><p className="text-white font-bold text-sm">Drag & Drop image to De-Brand</p><p className="text-zinc-500 text-xs mt-1">or click to browse files</p></div>
                 </div>
               )}
             </div>
             ...
             <div className="flex flex-col gap-4">
               <label className="text-zinc-400 font-black text-[11px] tracking-widest uppercase flex items-center justify-between">
                 IMAGE ASPECT RATIO
                 {detectedAR && file && <span className="text-orange-500 text-[9px] bg-orange-500/10 px-2 py-1 rounded border border-orange-500/30">LOCKED: {detectedAR}</span>}
               </label>
               ...
               <div className="mt-2 text-center p-3 border border-orange-500/20 rounded-xl bg-orange-500/5">
                 <p className="text-orange-400 font-bold uppercase tracking-widest text-[10px] md:text-[11px]">
                   Generate unbranded image in your favorite generator (Freepik, Nano Banana 2). Format is set to <span className="text-white font-black">DEFAULT</span>.
                 </p>
               </div>
             </div>
          </div>
          ...
          <div className="flex flex-col gap-6 h-full">
             <div className="flex items-center justify-between">
                <label className="text-emerald-500 font-black text-[11px] tracking-widest uppercase flex items-center gap-2">
                  <Code size={14} /> 2. DE-BRANDED JSON CODE
                </label>
             </div>
...
export default V8JsonDeBrendingExtractorPage;
// KRAJ FAJLA: V8JsonDeBrendingPage.jsx