...
import { Settings, Home, Layout, Zap, Image as ImageIcon, Box, Lock, ChevronDown, Store, FileText, PenTool, Crown, Upload, RefreshCcw, CheckCircle, Globe, Pizza, Save, Download, Flame, Anchor, Wine, Coffee, Utensils, Cake, Fish, Leaf, Droplets, ChefHat, Moon, CreditCard } from 'lucide-react';
...
export default function PremiumMenu() {
  ...
  // 🔥 STATE ZA KORISNIKA I PRETPLATU 🔥
  const [currentUser, setCurrentUser] = useState(null);
  const [isAdmin, setIsAdmin] = useState(false);
  const [isSubscribed, setIsSubscribed] = useState(false);
  const [isLoadingAuth, setIsLoadingAuth] = useState(true);

  // 1. Osluškivanje korisnika i MOMENTALNA PROVERA PRETPLATE
  useEffect(() => {
    window.scrollTo(0, 0);
        ...
        if (user) {
          ...
          if (!adminCheck) {
            try {
              ...
              if (!userSnap.exists()) {
                // Ako je nov korisnik, ODMAH ZAKLJUČAVAMO!
                await setDoc(userRef, { isSubscribed: false }, { merge: true });
                setIsSubscribed(false);
              } else {
                // Provera da li je pretplaćen
                const data = userSnap.data();
                setIsSubscribed(data.isSubscribed || false);
              }
            } catch (dbError) {
              console.error("Firebase greška:", dbError);
              setIsSubscribed(false); 
            }
          }
        } else {
  ...
  // 🔥 1. EKRAN AKO KORISNIK NIJE ULOGOVAN 🔥
  if (!currentUser) {
    return (
      <div className="min-h-screen bg-black text-white flex flex-col items-center justify-center p-6 relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-xl text-center bg-[#0a0f1c] border border-blue-500/30 p-10 rounded-[2.5rem] shadow-[0_0_50px_rgba(37,99,235,0.2)]">
          ...
          <p className="text-zinc-400 text-sm leading-relaxed mb-8">
            Please link your Google account to access your V8 QR Menu Builder dashboard.
          </p>
  ...
  // Određujemo da li je korisnik ZAKLJUČAN (Nije admin i nije platio)
  const isLockedOut = !isAdmin && !isSubscribed;
  ...
  return (
    <PayPalScriptProvider options={paypalOptions}>
      <div className="relative min-h-screen bg-black text-white font-sans selection:bg-orange-500/30">
        ...
        {/* 🔥 2. ZAKLJUČAN EKRAN (PAYWALL OVERLAY) KOJI LEBDI PREKO SAJTA 🔥 */}
        <AnimatePresence>
          {isLockedOut && (
            <motion.div 
              initial={{ opacity: 0 }} 
            ...
            >
              <div className="bg-[#050914]/95 border border-cyan-500/40 rounded-[3rem] p-8 md:p-14 text-center max-w-2xl w-full shadow-[0_0_80px_rgba(6,182,212,0.3)] relative overflow-hidden pointer-events-auto">
                ...
                <h2 className="text-3xl md:text-5xl font-black text-white uppercase tracking-widest mb-4 leading-tight">
                  ACCESS <span className="text-cyan-500">LOCKED</span>
                </h2>
                
                <p className="text-zinc-300 text-sm md:text-base leading-relaxed mb-8 max-w-lg mx-auto">
                  To unlock the V8 QR Menu Builder, generate unlimited menus, and host live digital catalogs, please activate your monthly subscription.
                </p>

                {/* 🔥 KARTICA SA JASNO ISPISANIM 200/mo 🔥 */}
                <div className="bg-black/50 border border-white/10 rounded-2xl p-6 mb-8 inline-block shadow-inner w-full max-w-md">
                  ...
                  <div className="text-5xl md:text-6xl font-black text-white flex justify-center items-center tracking-tight">
                    $200<span className="text-2xl text-cyan-400 font-bold ml-1">/mo</span>
                  </div>
                </div>
