...
const VaultGrid = () => {
  ...
  // POCETAK FUNKCIJE: useEffect (Auth & Fetch)
  useEffect(() => {
  ...
  // KRAJ FUNKCIJE: useEffect (Auth & Fetch)

  // POCETAK FUNKCIJE: fetchProjects
  const fetchProjects = async () => {
    try {
      ...
      if (!snap.empty) {
        ...
        // 🔥 TVOJ MANUELNI REDOSLED PROJEKATA (PO ID-u) 🔥
        const targetOrder = [
          "esmerald-project",     // 1. Mesto (Ako si ga snimio kao obsidian-emerald, promeni ovde)
          "obsidian-emerald",     // 1. Mesto (Fallback alternativa)
          "tom-foord-oud-wood",   // 2. Mesto
          "creed-adventus"        // 3. Mesto
        ];

        // Logika za forsirano sortiranje
        fetched.sort((a, b) => {
          const indexA = targetOrder.indexOf(a.id);
          const indexB = targetOrder.indexOf(b.id);

          if (indexA !== -1 && indexB !== -1) return indexA - indexB; // Oba su na listi, sortiraj po listi
          if (indexA !== -1) return -1; // Samo A je na listi, guraj ga gore
          if (indexB !== -1) return 1;  // Samo B je na listi, guraj ga gore
          return 0; // Nijedan nije na listi, zadrži originalni redosled
        });
  ...
  // KRAJ FUNKCIJE: fetchProjects

  // POCETAK FUNKCIJE: createSlug
  const createSlug = (text) => {
    return text.toLowerCase().trim().replace(/[\s\W-]+/g, '-');
  };
  // KRAJ FUNKCIJE: createSlug

  // POCETAK FUNKCIJE: handleThumbnailUpload
  ...
  // KRAJ FUNKCIJE: handleThumbnailUpload

  // POCETAK FUNKCIJE: handleCreateProject
  const handleCreateProject = async () => {
    ...
    try {
      ...
      // Osvezavamo listu nakon dodavanja da bi se primenio redosled
      fetchProjects(); 
  ...
  // KRAJ FUNKCIJE: handleCreateProject

  // POCETAK FUNKCIJE: handleDeleteProject
  ...
  // KRAJ FUNKCIJE: handleDeleteProject

  return (
    <div className="min-h-screen bg-[#020202] pt-24 pb-12 px-4 sm:px-8 relative selection:bg-[#ff6a00] selection:text-black">
      ...
      <div className="max-w-[1600px] mx-auto relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between border-b border-white/10 pb-6 mb-12">
          <div>
            ...
            <p className="text-zinc-500 text-xs font-bold uppercase tracking-[0.2em] flex items-center gap-2">
              <ShieldCheck className="w-4 h-4 text-[#ff6a00]" /> Experimental R&D Sector
            </p>
          </div>
          <div className="mt-4 md:mt-0 text-zinc-600 text-xs font-black uppercase tracking-widest">
            {isAdmin ? 'ADMIN ACCESS GRANTED' : `Showing: ${projects.length} Projects`}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 md:gap-8 mb-16 items-start">
          ...
          {projects.map((project, index) => (
            <div key={index} className={`group relative rounded-2xl overflow-hidden bg-[#0a0a0a] border border-white/5 hover:border-[#ff6a00]/50 transition-all duration-300 block ${project.ratio || 'aspect-video'} shadow-xl`}>
              ...
              <Link to={`/ui-ux/project/${project.id}`} className="absolute inset-0 z-10 cursor-pointer">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black via-black/50 to-transparent p-6 flex flex-col justify-end">
                  <span className="text-[#ff6a00] text-[10px] font-black uppercase tracking-widest mb-1 drop-shadow-md">{project.engine}</span>
          ...
          {isAdmin && (
            <button 
              ...
              className="group relative rounded-2xl overflow-hidden bg-black/50 border-2 border-dashed border-[#ff6a00]/30 hover:border-[#ff6a00] transition-all duration-300 aspect-video flex flex-col items-center justify-center cursor-pointer shadow-[0_0_0_rgba(255,106,0,0)] hover:shadow-[0_0_40px_rgba(255,106,0,0.15)]"
            >
              <div className="w-16 h-16 rounded-full bg-[#ff6a00]/10 group-hover:bg-[#ff6a00] flex items-center justify-center transition-colors duration-300 mb-4">
                <Plus className="w-8 h-8 text-[#ff6a00] group-hover:text-black transition-colors" />
              </div>
        ...
        <div className="flex justify-center items-center gap-2">
          <button className="w-10 h-10 flex items-center justify-center rounded-xl bg-black border border-zinc-800 text-zinc-500 hover:border-[#ff6a00] hover:text-[#ff6a00] transition-colors"><ChevronLeft className="w-4 h-4" /></button>
          <button className="w-10 h-10 flex items-center justify-center rounded-xl bg-[#ff6a00] border border-[#ff6a00] text-black font-black text-xs transition-colors">1</button>
          <button className="w-10 h-10 flex items-center justify-center rounded-xl bg-black border border-zinc-800 text-zinc-500 hover:border-[#ff6a00] hover:text-[#ff6a00] transition-colors"><ChevronRight className="w-4 h-4" /></button>
        </div>
      </div>
        ...
        {isModalOpen && isAdmin && (
          ...
          >
            <motion.div 
              ...
              className="bg-[#0a0a0a] border border-[#ff6a00]/30 rounded-3xl p-6 md:p-8 w-full max-w-lg shadow-[0_0_100px_rgba(255,106,0,0.15)] relative"
            >
              ...
              <div className="flex items-center gap-3 mb-8">
                <div className="w-10 h-10 rounded-full bg-[#ff6a00]/10 flex items-center justify-center"><Plus className="w-5 h-5 text-[#ff6a00]" /></div>
              ...
              <div className="flex flex-col gap-5">
                <div className="flex flex-col gap-2">
                  ...
                  <input 
                    ...
                    className="bg-[#050505] border border-white/10 rounded-xl p-4 text-white focus:border-[#ff6a00] outline-none transition-colors" 
                ...
                <div className="flex flex-col gap-2">
                  <label className="text-[10px] font-black uppercase tracking-widest text-zinc-500 flex justify-between">
                    ...
                    <span className="text-[#ff6a00]">Auto Upload Active</span>
                  </label>
                  <div className="flex flex-col sm:flex-row gap-3">
                    <input 
                      ...
                      className="flex-1 bg-[#050505] border border-white/10 rounded-xl p-4 text-white focus:border-[#ff6a00] outline-none transition-colors" 
                      placeholder="https://..." 
                    />
                    <label className="shrink-0 flex items-center justify-center gap-2 bg-zinc-800 hover:bg-zinc-700 cursor-pointer text-white px-6 py-4 rounded-xl font-black uppercase tracking-widest text-[10px] transition-colors">
                      {isUploadingThumbnail ? <Loader2 className="w-4 h-4 animate-spin text-[#ff6a00]" /> : <UploadCloud className="w-4 h-4" />}
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className="relative">
                    <select 
                      ...
                      className="w-full bg-[#050505] border border-white/10 rounded-xl p-4 text-white focus:border-[#ff6a00] outline-none transition-colors appearance-none cursor-pointer"
                    >
                      ...
                      <option value="aspect-square">1:1 (Square) - Za proizvode</option>
                      <option value="aspect-[4/5]">4:5 (Portrait) - Instagram stil</option>
                ...
                <button 
                  ...
                  className="mt-4 bg-gradient-to-r from-[#ff6a00] to-orange-500 hover:from-orange-500 hover:to-amber-500 disabled:opacity-50 text-black font-black uppercase tracking-widest p-4 rounded-xl flex items-center justify-center gap-2 transition-all shadow-[0_0_20px_rgba(255,106,0,0.4)] hover:shadow-[0_0_30px_rgba(255,106,0,0.6)]"
                >
                  {isSaving ? <div className="w-5 h-5 border-2 border-black/30 border-t-black rounded-full animate-spin"></div> : <><Save className="w-5 h-5" /> Initialize Project</>}
                </button>
              </div>
