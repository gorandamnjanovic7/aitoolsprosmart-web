...
import { ArrowLeft, Cpu, MonitorPlay, Zap, ShieldCheck, Layers, Plus, Save, X, Image as ImageIcon, Trash2, UploadCloud, Loader2, ChevronDown } from 'lucide-react';
...
const V10SplitScreen = () => {
        ...
        if (docSnap.exists()) {
          ...
          if (projectId === "obsidian-emerald") {
            setProjectData({
              ...
              description: "Case Study: An engineering sequence demonstrating emergence from absolute darkness, high-tech sci-fi interface integration, and the establishment of physical weight through contact shadows on a V8 obsidian pedestal.",
              phases: [
                { title: "Phase 1: The Awakening", subtitle: "Initial exposure and reflection establishment", imageUrl: "" },
                { title: "Phase 2: System Online", subtitle: "UI/HUD Hologram Integration", imageUrl: "" },
                { title: "Phase 3: Pedestal Lockdown", subtitle: "Physical weight and obsidian mechanics", imageUrl: "" }
              ],
              finalVideo: ""
            });
          } else {
  ...
  const handleCloudinaryUpload = async (e, type = 'image') => {
    ...
    if (file.size > 100 * 1024 * 1024) {
      alert("V10 Error: Video size exceeds 100MB limit. Please compress the file.");
      return;
    }
    ...
    try {
      ...
      if (data.secure_url) {
        ...
        alert(`Upload rejected by Cloudinary: ${data.error?.message || 'Unknown error'}`);
      }
    } catch (error) {
      console.error("Upload failed:", error);
      alert("System error: Could not connect to Cloudinary servers.");
    } finally {
  ...
  const handleSavePhase = async () => {
    ...
    try {
      ...
      const phaseData = {
        ...
        imageUrl: newPhase.imageUrl,
        ratio: newPhase.ratio, 
        createdAt: new Date().toISOString()
      };
      const updatedPhases = [...(projectData.phases || []), phaseData];
      await setDoc(docRef, {
        title: projectData.title,
        description: projectData.description,
        phases: updatedPhases
  ...
  const handleDeletePhase = async (indexToDelete) => {
    ...
    try {
      ...
      console.error("Greška pri brisanju faze:", error);
      alert("Sistemska greška: Nije moguće obrisati fazu.");
    }
  };

  const handleSaveVideo = async () => {
    if (!newVideoUrl) return alert("Please enter a video URL or upload a file!");
  ...
  const handleDeleteVideo = async () => {
    if(!window.confirm("Are you sure you want to remove the video?")) return;
    try {
      const docRef = doc(db, "v10_projects", projectId);
            ...
            <div className="flex justify-between items-start md:items-center">
              ...
              {isAdmin && (
                <button 
                  ...
                  className="bg-red-600/10 hover:bg-red-600 text-red-500 hover:text-white p-2.5 rounded-xl transition-all duration-300 border border-red-500/20 hover:border-red-500 shadow-[0_0_15px_rgba(220,38,38,0.1)] hover:shadow-[0_0_20px_rgba(220,38,38,0.4)] opacity-0 group-hover:opacity-100"
            ...
            <div className={`w-full ${phase.ratio || 'aspect-video'} bg-zinc-950 rounded-2xl md:rounded-[2rem] border border-white/5 overflow-hidden shadow-2xl relative`}>
               {phase.imageUrl ? (
                 ...
                 <div className="absolute inset-0 flex flex-col items-center justify-center gap-4">
                   {uploadingPhaseIndex === index ? (
                     <Loader2 className="w-12 h-12 text-orange-500 animate-spin" />
                   ) : (
                     <ImageIcon className="w-12 h-12 text-zinc-800" />
                   )}
                   <span className="text-zinc-700 font-black uppercase tracking-[0.3em] text-sm md:text-xl">
                     {uploadingPhaseIndex === index ? 'UPLOADING...' : 'IMAGE PLACEHOLDER'}
                   </span>
                   ...
                   {isAdmin && uploadingPhaseIndex !== index && (
                     <label className="mt-6 flex items-center gap-2 bg-orange-600/10 hover:bg-orange-600 text-orange-500 hover:text-black px-6 py-3 rounded-full cursor-pointer transition-colors border border-orange-500/30 hover:border-orange-600 shadow-[0_0_20px_rgba(249,115,22,0.1)]">
                       ...
                       <span className="text-xs font-black uppercase tracking-widest">Dodaj Sliku</span>
              ...
              <AnimatePresence>
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="flex flex-col gap-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="flex flex-col gap-2">
                      <label className="text-[10px] font-black uppercase tracking-widest text-zinc-500">Subtitle / Description</label>
                  ...
                  <div className="flex flex-col gap-2">
                    <label className="text-[10px] font-black uppercase tracking-widest text-zinc-500">Phase Image Format (Ratio)</label>
                  ...
                  <button onClick={handleSavePhase} disabled={isSavingPhase || isUploadingImage} className="mt-4 bg-orange-600 hover:bg-orange-500 disabled:opacity-50 text-white font-black uppercase tracking-widest p-4 rounded-xl flex items-center justify-center gap-2 transition-colors">
                  ...
                  <div className="w-full h-full p-4 md:p-8 flex items-center justify-center border-2 border-dashed border-orange-500/30 rounded-[2rem] hover:border-orange-500/50 transition-colors">
                    {!isAddingVideo ? (
                      <button onClick={() => setIsAddingVideo(true)} className="flex flex-col items-center gap-4 cursor-pointer group">
                        <div className="w-16 h-16 rounded-full border-2 border-orange-500/30 group-hover:border-orange-500 group-hover:bg-orange-500 text-orange-500 group-hover:text-black flex items-center justify-center transition-all duration-300">
                          <Plus className="w-8 h-8" />
                        </div>
                      ...
                      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-xl bg-black p-6 rounded-2xl border border-white/10 shadow-2xl">
                        ...
                        <div className="flex flex-col gap-3 mb-4">
                          <input 
                            ...
                            className="w-full bg-[#050505] border border-white/10 rounded-xl p-4 text-white focus:border-orange-500 outline-none" 
                            placeholder="Paste direct URL here..." 
                          />
                          <div className="flex items-center gap-4 w-full">
                            <div className="h-px bg-white/10 flex-1"></div>
                            <span className="text-[9px] font-black uppercase tracking-widest text-zinc-600">OR</span>
                            <div className="h-px bg-white/10 flex-1"></div>
                          </div>
                          <label className="w-full flex items-center justify-center gap-2 bg-zinc-800 hover:bg-zinc-700 cursor-pointer text-white px-6 py-4 rounded-xl font-black uppercase tracking-widest text-[10px] transition-colors border border-white/5 shadow-inner">
                            {isUploadingVideo ? <Loader2 className="w-5 h-5 animate-spin text-orange-500" /> : <UploadCloud className="w-5 h-5 text-zinc-400" />}
                            {isUploadingVideo ? 'Uploading Media...' : 'Upload Video File (.mp4)'}
                            <input type="file" accept="video/mp4,video/webm" className="hidden" onChange={(e) => handleCloudinaryUpload(e, 'video')} />
                          </label>
                        </div>
