...
import { Zap, DownloadCloud, Diamond, Aperture, ShieldCheck, Edit, Trash2 } from 'lucide-react';
...
export default function V10UltraAncientAssets({ 
  ...
  if (!paketi || paketi.length === 0) {
    return <div className="w-full text-center py-20 text-slate-400 font-black uppercase tracking-widest">Awaiting Ancient Assets. Radar is clear.</div>;
  }
  ...
  return (
    <>
      {paketi.map((paket) => {
        ...
        return (
          <div key={paket.id} className="relative w-full lg:w-[calc(50%-1.5rem)] p-[3px] rounded-[2rem] md:rounded-[3rem] overflow-hidden group/wrap transition-all duration-500 hover:shadow-[0_20px_50px_rgba(245,158,11,0.2)] flex flex-col bg-slate-100">
            ...
            <div className="relative z-10 bg-white/90 backdrop-blur-xl rounded-[calc(2rem-3px)] md:rounded-[calc(3rem-3px)] overflow-hidden h-full flex flex-col shadow-inner">
              ...
              <div className="p-3 md:p-5 relative bg-slate-50/50">
                {paket.volume && (
                  <div className="absolute top-5 left-5 md:top-8 md:left-8 z-10 bg-gradient-to-r from-amber-500 to-orange-400 text-white text-[8px] md:text-[10px] font-black uppercase tracking-widest px-2.5 py-1 md:px-3 md:py-1.5 rounded-full shadow-md border border-amber-300">
                    {paket.volume}
                  </div>
                )}
                <div className="absolute top-5 right-5 md:top-8 md:right-8 z-10 flex flex-col items-end gap-1.5 md:gap-2 max-w-[60%] md:max-w-[50%]">
                    <div className="bg-gradient-to-r from-amber-500 to-orange-500 text-white text-[8px] md:text-[10px] font-black uppercase tracking-widest px-2.5 py-1 md:px-3 md:py-1.5 rounded-full shadow-md text-center border border-amber-400">
                      150MP ANCIENT
                    </div>
                    {paket.kategorijaEn && (
                ...
                <div className="w-full aspect-[16/9] rounded-[1rem] md:rounded-2xl overflow-hidden cursor-pointer relative group border border-slate-200 v8-glass-container bg-slate-100 shadow-inner" onClick={() => setFullScreenImageUrl(paket.previewUrl)}>
                    ...
                    <div className="absolute inset-0 bg-white/20 backdrop-blur-[2px] opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center z-20">
                      <Zap className="text-amber-500 w-8 h-8 md:w-12 md:h-12 drop-shadow-md" />
                    </div>

                    {/* Vremenska narandžasta munja (pulsira na svakih 7 sekundi) */}
                    <motion.div 
                    ...
                    >
                       <Zap className="text-orange-400 w-10 h-10 md:w-16 md:h-16 drop-shadow-lg" fill="rgba(251,146,60,0.3)" strokeWidth={1.5} />
                    </motion.div>
                </div>

                {/* 🔥 GALERIJA MALIH SLIKA (THUMBNAILS) 🔥 */}
                  ...
                  <div className="grid grid-cols-5 gap-1.5 md:gap-2 mt-2 md:mt-3">
                    {paket.primeri.slice(0, 5).map((imgUrl, idx) => (
                      <div 
                        ...
                        className="aspect-square rounded-lg md:rounded-xl overflow-hidden cursor-pointer relative group border border-slate-200 v8-glass-container bg-slate-50"
                      >
                         <motion.img 
                           src={imgUrl} 
                           alt={`Preview ${idx + 1}`} 
              ...
              {/* TEKST I KONTROLE - Prilagođeno za mobilne */}
              <div className="p-5 md:p-8 pt-4 flex flex-col flex-grow bg-white/50">
                <h3 className="text-[16px] md:text-[22px] leading-tight font-black uppercase text-slate-900 mb-3 md:mb-4 tracking-tighter flex items-start gap-2 md:gap-3 drop-shadow-sm">
                    <Aperture className="text-amber-500 shrink-0 mt-0.5 md:mt-1 w-5 h-5 md:w-6 md:h-6" />
                    <span>{paket.nazivEn}</span>
                </h3>

                {/* V10 ENGINE BADGE */}
                <div className="bg-amber-50/50 border border-amber-100 rounded-lg md:rounded-xl p-2.5 md:p-3 mb-2.5 md:mb-3 flex items-center gap-2 shadow-sm">
                    <Aperture size={12} className="text-amber-500 shrink-0 md:w-[14px] md:h-[14px]" />
                    <span className="text-[7.5px] md:text-[10px] text-amber-700 font-black uppercase tracking-widest">150 MEGAPIXELS (V10 ENGINE)</span>
                </div>
                ...
                {/* DUGME I CENA - Flex col za telefone */}
                    ...
                    <button 
                      ...
                      className={`w-full sm:w-auto px-6 py-3.5 md:px-6 md:py-4 rounded-xl font-black text-[11px] md:text-[13px] uppercase tracking-widest transition-all flex items-center justify-center gap-2 shadow-md hover:scale-105 ${
                        (!isAdmin && isOwned) 
                          ...
                          : 'bg-gradient-to-r from-amber-500 to-orange-500 text-white hover:shadow-[0_10px_20px_rgba(245,158,11,0.3)]'
                      }`}
                    >
