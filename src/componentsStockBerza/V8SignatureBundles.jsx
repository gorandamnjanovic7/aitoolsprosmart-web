  ...
  return (
    <>
      {paketi.map((paket) => {
        ...
        return (
          <div key={paket.id} className="bg-[#0a0a0a] rounded-[2.5rem] border border-yellow-500/20 overflow-hidden shadow-[0_0_30px_rgba(245,158,11,0.05)] hover:shadow-[0_0_40px_rgba(245,158,11,0.15)] transition-all flex flex-col relative w-full lg:w-[calc(50%-1.5rem)]">
            
            {/* SLIKA I ZNAČKE (BADGES) */}
            <div className="p-4 md:p-5 relative">
               {paket.volume && (
                 <div className="absolute top-8 left-8 z-10 bg-gradient-to-r from-yellow-600 to-amber-500 text-black text-[10px] font-black uppercase tracking-widest px-3 py-1.5 rounded-full shadow-lg">
                   {paket.volume}
                 </div>
               )}
               <div className="absolute top-8 right-8 z-10 flex flex-col items-end gap-2">
                  <div className="bg-gradient-to-r from-yellow-600 to-amber-500 text-black text-[10px] font-black uppercase tracking-widest px-3 py-1.5 rounded-full shadow-lg">
                    60MP SIGNATURE
                  </div>
                  {paket.kategorijaEn && (
                    <div className="bg-black/80 backdrop-blur-md border border-yellow-500/50 text-yellow-300 text-[9px] font-black uppercase tracking-widest px-3 py-1.5 rounded-full">
                      {paket.kategorijaEn}
                    </div>
                  )}
               </div>
               ...
               <div className="w-full aspect-[16/9] rounded-2xl overflow-hidden cursor-pointer relative group border border-white/5" onClick={() => setFullScreenImageUrl(paket.previewUrl)}>
                  ...
                  >
                     <Zap className="text-yellow-400 w-16 h-16 drop-shadow-[0_0_40px_rgba(245,158,11,1)]" fill="rgba(245,158,11,0.3)" strokeWidth={1.5} />
                  </motion.div>
                  ...
                  <div className="grid grid-cols-4 gap-3 mt-3">
                     {paket.primeri.slice(0, 4).map((thumb, idx) => (
                        <div key={idx} className="aspect-square rounded-xl overflow-hidden cursor-pointer relative group border border-white/5" onClick={() => setFullScreenImageUrl(thumb)}>
                           ...
                           />
                           <div className="absolute inset-0 bg-yellow-500/20 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none" />
                        </div>
                     ))}
                  </div>
               )}
            ...
            <div className="p-6 md:p-8 pt-2 flex flex-col flex-grow">
               ...
               <h3 className="text-xl md:text-[22px] leading-tight font-black uppercase text-white mb-5 tracking-widest flex items-start gap-3">
                  <Diamond className="text-yellow-500 shrink-0 mt-0.5" size={24} />
                  <span>{paket.nazivEn}</span>
               </h3>

               <div className="bg-yellow-900/10 border border-yellow-500/20 rounded-xl p-3 mb-3 flex items-center gap-2">
                  <Diamond size={14} className="text-yellow-400 shrink-0" />
                  <span className="text-[9px] md:text-[10px] text-yellow-300 font-black uppercase tracking-widest">60 MEGAPIXELS OMNI-CHANNEL</span>
               </div>
               ...
               <div className="flex items-end justify-between mt-auto pt-6 border-t border-white/5">
                  <div>
                     ...
                     <p className="text-3xl md:text-4xl font-black text-yellow-400 drop-shadow-md">${getGlobalCena(paket.cena)}</p>
                  </div>
                  ...
                  <button 
                    ...
                    className={`px-6 py-4 rounded-xl font-black text-[11px] md:text-[13px] uppercase tracking-widest transition-all flex items-center gap-2 hover:scale-105 ${
                      (!isAdmin && isOwned) 
                        ...
                        : 'bg-gradient-to-r from-yellow-600 to-amber-500 text-black shadow-[0_0_20px_rgba(245,158,11,0.3)]'
                    }`}
                  >
