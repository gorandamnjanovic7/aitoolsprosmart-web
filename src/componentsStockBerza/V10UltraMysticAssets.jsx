  ...
  return (
    <>
      {paketi.map((paket) => {
        ...
        return (
          <div key={paket.id} className="relative w-full lg:w-[calc(50%-1.5rem)] p-[3px] rounded-[2rem] md:rounded-[3rem] overflow-hidden group/wrap transition-all duration-500 hover:shadow-[0_20px_50px_rgba(168,85,247,0.2)] flex flex-col bg-slate-100">
            
            {/* 🔥 TAČNA GEMINI ANIMACIJA IZ VIDEA (Uvek vidljiva 100% - Ljubičasto-Pink) 🔥 */}
            ...
            <div className="relative z-10 bg-white/90 backdrop-blur-xl rounded-[calc(2rem-3px)] md:rounded-[calc(3rem-3px)] overflow-hidden h-full flex flex-col shadow-inner">
              ...
              <div className="p-3 md:p-5 relative bg-slate-50/50">
                 {paket.volume && (
                   <div className="absolute top-5 left-5 md:top-8 md:left-8 z-10 bg-gradient-to-r from-fuchsia-500 to-pink-400 text-white text-[8px] md:text-[10px] font-black uppercase tracking-widest px-2.5 py-1 md:px-3 md:py-1.5 rounded-full shadow-md border border-fuchsia-300">
                     {paket.volume}
                   </div>
                 )}
                 <div className="absolute top-5 right-5 md:top-8 md:right-8 z-10 flex flex-col items-end gap-1.5 md:gap-2 max-w-[60%] md:max-w-[50%]">
                    <div className="bg-gradient-to-r from-purple-500 to-pink-500 text-white text-[8px] md:text-[10px] font-black uppercase tracking-widest px-2.5 py-1 md:px-3 py-1.5 rounded-full shadow-md text-center border border-purple-400">
                       ...
                       {paket.primeri.slice(0, 5).map((thumb, idx) => (
                          <div key={idx} className="aspect-square rounded-lg md:rounded-xl overflow-hidden cursor-pointer relative group border border-slate-200 v8-glass-container bg-slate-50" onClick={() => setFullScreenImageUrl(thumb)}>
                             ...
                             <motion.img 
                               ...
                               // Trajanje svake male slike je drugačije (5s, 6s, 7s, 8s, 9s) da ne dišu sve u istom trenutku!
