// POČETAK FAJLA: src/V8StockBerza.jsx
...
import { collection, addDoc, deleteDoc, doc, updateDoc, serverTimestamp, query, getDoc, setDoc, where, onSnapshot } from 'firebase/firestore';
...
const V8StockBerza = () => {
  ...
  const [activeTab, setActiveTab] = useState(() => {
    let tab = localStorage.getItem('v8_active_stock_tab') || 'bundles';
    if (tab === 'standard') tab = 'bundles'; 
    return tab;
  }); 

  useEffect(() => {
    if (activeTab === 'ultra150_2' || activeTab === 'ultra150_3' || activeTab === 'ancient') {
      setActiveTab('ultra150');
    }
  ...
  const [noviFormat, setNoviFormat] = useState('45MP MASTERWORK BUNDLE');
  ...
  useEffect(() => {
    ...
    const unsubPaketi = onSnapshot(q, (snap) => {
        ...
        let ucitani = snap.docs.map(d => {
            ...
            else if (fmt.includes('60') || title.includes('60') || fmt.includes('SIGNATURE')) { 
                p.format = '60MP SIGNATURE BUNDLE'; 
            } 
            else if (fmt.includes('33') || title.includes('33') || fmt.includes('SINGLE')) { 
                p.format = '33.2MP MASTERWORK SINGLE'; 
            } 
            else { 
        ...
        ucitani.sort((a, b) => {
            ...
            const timeA = getMs(a.createdAt) || getMs(a.updatedAt) || 0;
            const timeB = getMs(b.createdAt) || getMs(b.updatedAt) || 0;
            return timeB - timeA;
        });

        setPaketi(ucitani);
    }, (error) => {
        console.error("🚨 [V8 SYSTEM] Firestore onSnapshot Error:", error);
    });
        ...
        if(trazeniPaket) {
            try {
                ...
                if (trazeniPaket.paddleLink && trazeniPaket.paddleLink.trim() !== "") { window.location.href = trazeniPaket.paddleLink; } 
                  ...
                  setCheckoutData({ isOpen: true, name: fullName, price: getGlobalCena(trazeniPaket.cena), zipLink: trazeniPaket.zipLink }); 
                }
  ...
  useEffect(() => {
    ...
    if (!editingPaketId) {
      ...
      else if (activeTab === 'premium') setNoviFormat('33.2MP MASTERWORK SINGLE');
      else setNoviFormat('45MP MASTERWORK BUNDLE');
    }
  ...
  const startEditPaket = (paket) => { setEditingPaketId(paket.id); setNoviNazivEn(paket.nazivEn || ''); setNoviVolume(paket.volume || ''); setNoviFormat(paket.format || '45MP MASTERWORK BUNDLE'); setNovaKategorijaEn(paket.kategorijaEn || ''); setNovaCena(paket.cena || '49.99'); setNoviOpisEn(paket.opisEn || ''); setPreviewUrl(paket.previewUrl || ''); setZipLink(paket.zipLink || ''); setIsFree(paket.isFree || false); setPrimeriUrls(paket.primeri || []); window.scrollTo({ top: 0, behavior: 'smooth' }); };
  const stoziEdit = () => { setEditingPaketId(null); setNoviNazivEn(''); setNoviVolume(''); setNoviFormat('45MP MASTERWORK BUNDLE'); setNovaKategorijaEn(''); setNovaCena('49.99'); setPreviewUrl(''); setZipLink(''); setIsFree(false); setPrimeriUrls([]); };
