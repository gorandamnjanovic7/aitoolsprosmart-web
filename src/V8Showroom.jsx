...
const FullScreenLightbox = ({ item, onClose }) => {
    if (!item) return null;
    return (
        <div className="fixed inset-0 z-[10000] bg-black/95 backdrop-blur-3xl flex items-center justify-center p-4 cursor-pointer" onClick={onClose}>
            ...
            <div className="relative max-w-7xl w-full flex flex-col items-center" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="mt-6 text-center">
                    <h3 className="text-[#FF8C00] font-black text-2xl uppercase tracking-widest">{item.title}</h3>
...
const V8Showroom = () => {
    ...
    return (
        <div className="min-h-screen bg-[#050505] pt-32 pb-24 px-6 font-sans text-white">
            <FullScreenLightbox item={lightboxItem} onClose={() => setLightboxItem(null)} />

            {/* HERO SEKCIJA */}
            <div className="max-w-7xl mx-auto text-center mb-20 relative">
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-3/4 h-32 bg-[#FF8C00] opacity-20 blur-[120px] rounded-full pointer-events-none"></div>
            ...
            {/* FILTERI */}
            <div className="flex flex-wrap justify-center gap-4 mb-16 max-w-5xl mx-auto relative z-10">
                {filters.map(filter => (
                    <button 
                        key={filter}
                        onClick={() => setActiveFilter(filter)}
                        className={`px-8 py-3.5 rounded-xl font-black text-[11px] uppercase tracking-widest transition-all duration-300 border ${activeFilter === filter ? 'bg-[#FF8C00]/20 border-[#FF8C00] text-[#FF8C00] shadow-[0_0_20px_rgba(255,140,0,0.2)]' : 'bg-[#0a0a0a] border-white/10 text-zinc-500 hover:border-[#FF8C00]/50 hover:text-white'}`}
                    >
                        {filter}
                    </button>
                ))}
            ...
            {/* MASONRY/GRID GALERIJA */}
            <div className="columns-1 md:columns-2 lg:columns-3 gap-6 space-y-6 max-w-7xl mx-auto">
                {filteredItems.map(item => (
                    <div 
                        key={item.id} 
                        onClick={() => setLightboxItem(item)}
                        className="relative group rounded-3xl overflow-hidden bg-[#0a0a0a] border border-white/5 cursor-pointer break-inside-avoid transform transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_20px_50px_rgba(255,140,0,0.15)]"
                    >
                        {/* Format i Tip Bedževi */}
            ...
            <div className="max-w-4xl mx-auto mt-32 text-center bg-gradient-to-b from-[#0a0a0a] to-[#050505] border border-white/5 rounded-[3rem] p-12 relative overflow-hidden">
