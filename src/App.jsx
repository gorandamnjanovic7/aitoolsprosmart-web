...
// --- V8 CINEMATIC LOADER (KRUŽNI PROGRES OKO LOGOA) ---
    ...
    <div className="fixed inset-0 z-[9999] bg-[#050505] flex flex-col items-center justify-center">
      <div className="relative flex items-center justify-center mb-8">
        <svg className="w-56 h-56 transform -rotate-90 drop-shadow-[0_0_20px_rgba(249,115,22,0.3)]" viewBox="0 0 140 140">
          ...
          <circle cx="70" cy="70" r={radius} fill="transparent" stroke="#ea580c" strokeWidth="3" strokeDasharray={circumference} strokeDashoffset={strokeDashoffset} className="transition-all duration-300 ease-out" strokeLinecap="round" />
        </svg>
      ...
      <div className="flex flex-col items-center gap-3">
        ...
        <div className="text-zinc-500 font-mono text-[10px] tracking-[0.4em] flex items-center gap-3"><span className="w-1.5 h-1.5 rounded-full bg-green-500 animate-ping" /><span>ESTABLISHING UPLINK</span><span className="text-orange-500 font-black min-w-[30px]">{Math.min(progress, 100)}%</span></div>
      </div>
    </div>
  );
};
...
function EnhancerPage() {
  ...
  const handleRollDice = (e) => { 
    ...
    if (customerPrompt.trim() !== "") { setExclusiveWarning("You must clear the 'Paste Customer Prompt' field first."); return; }
    setIsRolling(true);
  ...
  const handleEnhance = (e, boxType) => {
    ...
    setTimeout(() => { 
      try { 
        ...
        if (boxType === 'prompt') {
            let roastMsgs = [];
            if (detectedBadFormat) roastMsgs.push("- Your formatting was inefficient; I have purged the bracketed noise.");
            if (missingLighting) roastMsgs.push("- You ignored lighting; I injected precise volumetric protocols.");
            if (missingCamera) roastMsgs.push("- No optical system defined; I added specific lenses for realistic depth.");
            const roastIntro = `/// V8 AI EXPERT ANALYSIS ///\n\n🟢 WHAT'S GOOD:\nVision is solid.\n\n🔴 WHAT'S BAD:\n${roastMsgs.length > 0 ? roastMsgs.join('\n') : "Lacked technical depth Standard."}\n\n🚀 V8 10X FINAL PROMPT:\n\n`;
  ...
  const handleCopy = (text, boxName) => { 
    let copyText = text; if (text.includes("🚀 V8 10X FINAL PROMPT:\n\n")) copyText = text.split("🚀 V8 10X FINAL PROMPT:\n\n")[1];
  ...
  return (
    <div className="pt-32 pb-24 px-6 max-w-[1600px] mx-auto font-sans text-left text-white min-h-screen relative">
      ...
      <div className="mb-8 relative z-10"><Link to="/" className="text-zinc-400 hover:text-white flex items-center gap-2 uppercase text-[10px] font-black tracking-widest transition-all w-fit"><ChevronLeft className="w-4 h-4" /> System Registry</Link></div>
      ...
      <div className="flex flex-col gap-12 w-full items-stretch relative z-10">
         <div className="bg-[#0a0a0a]/50 backdrop-blur-md border border-blue-500/30 rounded-[2.5rem] p-8 md:p-12 relative flex flex-col gap-10 hover:border-blue-500/60 group">
            <div className="w-full text-center border-b border-blue-500/20 pb-6 mb-2"><h2 className="text-[8px] sm:text-[10px] md:text-[12px] font-black uppercase text-blue-400 tracking-wider">CONVERT YOUR IDEAS, ROLL THE DICE, OR UPLOAD IMAGE</h2></div>
            <div className="w-full flex flex-col lg:flex-row gap-8">
               <div className="w-full lg:w-1/3 flex flex-col justify-start text-left">
                 ...
                 <p className="text-[10px] md:text-[11px] text-white font-black uppercase tracking-widest mb-6">Enter your basic idea or roll the V8 Dice.</p>
               </div>
               <div className="w-full lg:w-2/3 relative flex flex-col rounded-2xl border border-white/10 bg-[#050505]/50 focus-within:border-blue-500/50">
                 ...
                 <textarea value={demoInput} spellCheck="false" data-gramm="false" data-lt-active="false" onChange={e => { if (customerPrompt.trim() !== "") { setExclusiveWarning("You must clear 'Paste Customer Prompt' first."); return; } setDemoInput(e.target.value); setGeneratedPrompts({ single: '', abstract: '', cinematic: '', photoreal: '', uniquePhoto: '' }); }} placeholder="e.g. 'a golden watch' or Upload Image" className={`w-full flex-1 bg-transparent pr-28 py-4 text-white text-[16px] font-medium outline-none resize-none min-h-[100px] ${uploadedImage ? 'pl-20' : 'pl-6'}`} />
            ...
            <div className="w-full flex flex-col border-t border-blue-500/20 pt-8">
               <label className="text-[10px] md:text-[12px] font-black uppercase text-blue-400 tracking-widest flex items-center gap-2 mb-6 border-b border-blue-500/20 pb-4"><Sparkles className="w-4 h-4 mr-1" /> V8 Cinematic Engine Output</label>
            ...
            <div className="w-full flex flex-col lg:flex-row justify-between items-center gap-8 mt-4 border-t border-blue-500/20 pt-8">
               <div className="flex flex-col sm:flex-row gap-8 w-full lg:w-auto text-left">
                  <div className="flex flex-col gap-4"><span className="text-[12px] font-black uppercase text-zinc-300">ASPECT RATIO</span><div className="flex flex-wrap gap-2">{['1:1', '9:16', '16:9', '21:9'].map(ar => <OptionButton key={`ar-${ar}`} label={ar} selected={selectedAR === ar} onClick={() => setSelectedAR(ar)} type="ar" />)}</div></div>
                  <div className="flex flex-col gap-4"><span className="text-[12px] font-black uppercase text-zinc-300">QUALITY</span><div className="flex flex-wrap gap-2">{['1x', '2x', '4x'].map(q => <OptionButton key={`q-${q}`} label={q} selected={selectedQuality === q} onClick={() => setSelectedQuality(q)} type="quality" />)}</div></div>
               </div>
               <button type="button" onClick={(e) => handleEnhance(e, 'concept')} disabled={isEnhancing || (!demoInput && !customerPrompt)} className="w-full lg:w-[30%] bg-gradient-to-r from-blue-700 to-blue-500 text-white py-6 rounded-2xl font-black uppercase text-[14px] transition-all flex items-center justify-center shadow-lg cursor-pointer">{isEnhancing ? <Loader2 className="w-6 h-6 animate-spin mr-4" /> : "Enhance Cinematic Concept"}</button>
            </div>
         </div>
         <div className="bg-[#0a0a0a]/50 backdrop-blur-md border border-amber-400/30 rounded-[2.5rem] p-8 md:p-12 hover:border-amber-400/60 group">
            <div className="w-full text-center border-b border-amber-400/20 pb-6 mb-2"><h2 className="text-[8px] sm:text-[10px] md:text-[12px] font-black uppercase text-amber-400 tracking-wider">WE WILL MAKE YOUR PROMPT PERFECT</h2></div>
            <div className="w-full flex flex-col lg:flex-row gap-8">
               <div className="w-full lg:w-1/3 text-left">
                 <label className="text-[14px] md:text-[16px] font-black uppercase text-amber-400 tracking-widest flex items-center gap-2 mb-3"><Zap className="w-5 h-5" /> Paste Customer Prompt</label>
                 <p className="text-[10px] md:text-[11px] text-white font-black uppercase tracking-widest mb-6">Paste raw prompt to rebuild it.</p>
               </div>
               <div className="w-full lg:w-2/3 relative flex flex-col overflow-hidden rounded-2xl border border-white/10 bg-[#050505]/50 focus-within:border-amber-400/50">
                 ...
                 <textarea value={customerPrompt} spellCheck="false" data-gramm="false" data-lt-active="false" onChange={e => { if (demoInput.trim() !== "" || uploadedImage) { setExclusiveWarning("You must clear 'Concept / Subject' first."); return; } setCustomerPrompt(e.target.value); setGeneratedPrompts({ single: '', abstract: '', cinematic: '', photoreal: '', uniquePhoto: '' }); }} placeholder="PASTE YOUR RAW PROMPT HERE" className="w-full flex-1 bg-transparent p-6 text-white text-[12px] md:text-[14px] outline-none resize-none min-h-[160px]" />
            ...
            <div className="w-full flex flex-col border-t border-amber-400/20 pt-8">
               ...
               <div className="w-full bg-[#0a0a0a]/50 border border-amber-400/20 rounded-[2rem] p-6 pb-20 relative min-h-[300px]">
                 ...
                 <div className="w-full font-mono text-[11px] md:text-[13px] leading-relaxed text-left text-zinc-200 whitespace-pre-wrap">{generatedPrompts.single ? <TypewriterText text={generatedPrompts.single} speed={10} /> : "AWAITING CORE INPUT..."}</div>
                 {generatedPrompts.single && <button type="button" onClick={() => handleCopy(generatedPrompts.single, 'single')} className="absolute bottom-6 right-6 px-6 py-3 rounded-xl text-[11px] font-black uppercase bg-amber-400/10 border border-amber-400/20 text-amber-300 hover:bg-amber-400 hover:text-black">Copy 10X Prompt</button>}
               </div>
            </div>
            <div className="w-full flex flex-col lg:flex-row justify-between items-center gap-8 mt-4 border-t border-amber-400/20 pt-8">
               <div className="flex flex-col sm:flex-row gap-8 w-full lg:w-auto text-left"><div className="flex flex-col gap-4"><span className="text-[12px] font-black text-amber-100 uppercase">QUALITY</span><div className="flex flex-wrap gap-2">{['1x', '2x', '4x'].map(q => <OptionButton key={`q2-${q}`} label={q} selected={selectedQuality === q} onClick={() => setSelectedQuality(q)} type="quality" />)}</div></div></div>
...
function SingleProductPage({ apps = [] }) {
  ...
  return (
    <div className="bg-[#050505] pt-32 pb-32 px-6 font-sans text-white text-left relative">
      ...
      {fullScreenImage && <div className="fixed inset-0 z-[6000] bg-black/95 flex items-center justify-center p-4" onClick={() => setFullScreenImage(null)}><button className="absolute top-6 right-6 text-white bg-black/50 hover:bg-red-600 rounded-full p-3 transition-all z-[6010]"><X className="w-8 h-8" /></button><img src={fullScreenImage} className="max-w-full max-h-full object-contain" alt="Enlarged" onClick={(e) => e.stopPropagation()} /></div>}
