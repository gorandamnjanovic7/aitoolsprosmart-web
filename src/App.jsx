...
const getRibbonStyle = (index) => {
  if (index === 0) return "bg-gradient-to-r from-orange-600 to-red-600 shadow-[0_0_15px_rgba(234,88,12,0.5)]";
  const colors = ["bg-blue-600", "bg-purple-600", "bg-emerald-600", "bg-pink-600", "bg-indigo-600"];
...
const OptionButton = ({ label, selected, onClick, type }) => {
  ...
  return (
    <button onClick={onClick} className={`px-4 py-2 rounded-lg text-[9px] font-black border transition-all ${selected ? activeClass : "bg-black border-white/10 text-zinc-500 hover:border-white/20"}`}>
      {label}
    </button>
  );
};
...
const FAQAccordion = ({ faqList }) => {
  ...
  return (
    <div className="mt-12 border-t border-white/5 pt-10">
      <h3 className="text-[12px] font-black text-white uppercase tracking-widest mb-6 border-l-4 border-orange-500 pl-4 italic flex items-center gap-3 text-left"><HelpCircle className="w-4 h-4 text-orange-500" /> FREQUENTLY ASKED QUESTIONS</h3>
        ...
        {faqList.filter(f => f.q && f.a).map((item, idx) => (
          <div key={idx} className="bg-white/[0.02] border border-white/5 rounded-2xl overflow-hidden shadow-inner text-left transition-all">
            <button onClick={() => setOpenIdx(openIdx === idx ? null : idx)} className="w-full p-6 flex justify-between items-center text-left hover:bg-white/[0.04] transition-colors">
              <h4 className="text-orange-500 font-bold text-[13px] uppercase tracking-wider flex items-center gap-3"><span className="text-white/30 text-[10px]">Q:</span> {item.q}</h4>
              <ChevronDown className={`w-5 h-5 text-zinc-500 transition-transform duration-300 ${openIdx === idx ? 'rotate-180' : ''}`} />
            </button>
            {openIdx === idx && <div className="p-6 pt-0 text-zinc-400 text-[12px] leading-relaxed border-t border-white/5 mt-2 pt-4"><span className="text-white/30 font-bold mr-2 text-[10px] uppercase">A:</span>{item.a}</div>}
          </div>
        ))}
      </div>
    </div>
  ...
  const handleEnhance = useCallback(() => {
    ...
    setTimeout(() => {
      try {
        ...
        const comboList = data.UNIQUE_PHOTOREAL_COMBOS || [];
        const keywordList = data.CAMERA_LENS_META_KEYWORDS || [];
        const poolList = data.UNIQUE_META_POOL || [];
        const combo = comboList[Math.floor(Math.random() * comboList.length)] || "";
        const metaKeys = keywordList[Math.floor(Math.random() * keywordList.length)] || "";
        const uniquePool = poolList.slice(0, 8).join(", ");
        const customPhotoreal = `ULTIMATE PHOTOREALISM CORE: ${subject}. ${combo}. ${metaKeys}. ${uniquePool}. Zero-noise crystalline clarity, absolute photographic truth.`;
        setGeneratedPrompts({ ...std, cctv: customPhotoreal.replace(/neon/gi, "vivid glowing").replace(/cyberpunk/gi, "high-tech futuristic") });
...
function HomePage({ apps = [] }) {
  const [activeSlide, setActiveSlide] = useState(0); const [liveVideos, setLiveVideos] = useState([]); const [isLoadingVideos, setIsLoadingVideos] = useState(true); 
  const location = useLocation();
  const sortedApps = [...apps].sort((a, b) => Number(b.id) - Number(a.id));
  useEffect(() => { 
    let isMounted = true; const fetchVideos = async () => {
      setIsLoadingVideos(true); const RSS_URL = encodeURIComponent(`https://www.youtube.com/feeds/videos.xml?channel_id=UC6ilBUks_oFMSD8CE9qD6lQ`);
      const endpoints = [`https://api.rss2json.com/v1/api.json?rss_url=${RSS_URL}`, `https://api.allorigins.win/raw?url=${RSS_URL}`];
      for (let url of endpoints) { try { const res = await fetch(url); if (!res.ok) continue; if (url.includes('rss2json')) { const result = await res.json(); if (result?.items?.length > 0) { if (isMounted) { setLiveVideos(result.items.slice(0, 8).map(item => ({ title: item.title, url: item.link }))); setIsLoadingVideos(false); } return; } } } catch (e) {} }
  ...
  return (
    <>
      ...
      <div id="home-banner" className="relative w-full h-[85vh] flex items-end overflow-hidden bg-black text-white">
        <MatrixRain />
        {(data.BANNER_DATA || []).map((item, idx) => (<div key={idx} className={`absolute inset-0 transition-opacity duration-1000 ${idx === activeSlide ? 'opacity-100' : 'opacity-0'}`}>{item.image && <img src={item.image} className="w-full h-full object-cover" alt="banner" />}<div className="absolute inset-x-0 bottom-0 h-1/2 bg-gradient-to-t from-[#050505] via-transparent z-10" /><div className="absolute bottom-0 left-0 w-full z-20" style={{ borderTop: '0.1px solid #f97316' }} /></div>))}
        ...
        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex gap-1.5 z-30">{(data.BANNER_DATA || []).map((_, i) => <button key={i} onClick={() => setActiveSlide(i)} className={`h-[1px] transition-all duration-500 rounded-full ${i === activeSlide ? 'w-6 bg-orange-500 shadow-[0_0_8px_rgba(249,115,22,0.6)]' : 'w-1.5 bg-white/20 hover:bg-white/40'}`} />)}</div>
        <div className="relative z-20 max-w-7xl mx-auto px-6 pb-20 w-full text-left font-sans"><div className="inline-block px-3 py-1 rounded-full bg-orange-600/90 text-[6px] font-black uppercase mb-4 tracking-widest">{data.BANNER_DATA?.[activeSlide]?.badge}</div><h1 className="text-xl md:text-4xl font-black uppercase mb-1.5 tracking-tighter transition-all duration-500">{data.BANNER_DATA?.[activeSlide]?.title}</h1><p className="text-zinc-300 text-[12px] md:text-sm max-w-lg font-medium opacity-90">{data.BANNER_DATA?.[activeSlide]?.subtitle}</p></div>
      </div>
      <div className="max-w-7xl mx-auto px-6 py-12 text-left">
        ...
        <div id="enhancer" className="mb-24 flex flex-col items-center justify-center text-center py-20 border-y border-orange-500/30 scroll-mt-32">
          ...
          <p className="text-zinc-400 text-[10px] md:text-[12px] max-w-2xl font-medium uppercase tracking-[0.2em] leading-relaxed mb-10 mx-auto">ACCESS THE PREMIUM AI PROMPT ENGINEERING ENGINE. CONVERT SIMPLE IDEAS INTO MASTERPIECES.<br /><br /><span className="text-orange-500 font-black uppercase">ENTER YOUR PROMPT; WE WILL ANALYZE IT IN DETAIL AND ENHANCE IT TO BE 10X BETTER, ENSURING YOU GET A 10X BETTER AI IMAGE.</span></p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 pb-32">{sortedApps.map((app, index) => <data.AssetCard key={app.id} app={app} ribbonStyle={getRibbonStyle(index)} />)}</div>
      </div>
    </>
  );
}
...
function SingleProductPage({ apps = [] }) {
  ...
  const currentMedia = app.media?.[activeMedia] || { url: data.bannerUrl, type: 'image' }; const isVideo = currentMedia?.type === 'video' || currentMedia?.url?.match(/\.(mp4|webm|ogg|mov)$/i); const { s: sysData } = data.extractSys(app.description); const parts = (app.whopLink || "").split("[SPLIT]");
  return (
        ...
        <div className="flex flex-col lg:flex-row gap-12 items-start">
          <div className="w-full lg:w-[70%]">
            <div className="aspect-video rounded-[2.5rem] overflow-hidden border border-orange-500 bg-black mb-6 relative group shadow-2xl cursor-pointer">
              {!isVideo ? (<><img src={currentMedia.url} onClick={() => setFullScreenImage(currentMedia.url)} className="w-full h-full object-cover" alt="" /><button className="absolute top-6 right-6 p-3 bg-black/60 rounded-xl opacity-0 group-hover:opacity-100 transition-all z-20"><Maximize2 className="w-4 h-4" /></button></>) : (<UniversalVideoPlayer videoRef={mainVideoRef} url={currentMedia.url} />)}
            ...
            <h1 className="text-xl md:text-2xl font-black uppercase tracking-tighter mt-8 mb-4">{app.name}</h1>
            ...
            {app.headline && <p className="text-sm md:text-base text-white font-black mb-10 border-l-4 border-orange-500 pl-5 italic">{app.headline}</p>}
            <div className="border-t border-white/5 pt-8 mb-12">{renderDescription(app.description)}<FAQAccordion faqList={app.faq} /></div>
          </div>
          <div className="w-full lg:w-[30%] lg:sticky lg:top-40"><div className="bg-[#0a0a0a] border border-white/10 rounded-[2.5rem] overflow-hidden shadow-2xl p-8"><img src={mojBaner} alt="" className="w-full h-32 object-cover rounded-2xl mb-8" /><div className="space-y-6 mb-8"><div className="relative rounded-2xl bg-white/[0.02] border border-white/10 py-3.5 flex items-center justify-center"><div className="absolute -top-3 px-4 py-1 rounded-full bg-blue-600 text-[8px] font-black uppercase tracking-widest shadow-lg">Monthly</div><span className="text-2xl font-black">${app.price || '14.99'}</span></div><div className="relative rounded-2xl bg-orange-500/[0.03] border border-orange-500/30 py-3.5 flex items-center justify-center mt-6"><div className="absolute -top-3 px-4 py-1 rounded-full bg-orange-600 text-[8px] font-black uppercase tracking-widest shadow-lg">Lifetime</div><span className="text-2xl font-black">${app.priceLifetime || '88.99'}</span></div></div><a href={data.formatExternalLink(sysData.w || parts[0])} target="_blank" rel="noreferrer" className="w-full py-4 rounded-xl flex items-center justify-center bg-blue-600 text-white font-black text-[10px] uppercase tracking-widest hover:bg-blue-500 transition-all shadow-xl">Unlock On Whop</a></div></div>
        </div>
      </div>
    </div>
  );
...
function AdminPage({ apps = [], refreshData }) {
  ...
  const handleEditClick = (app) => { const parts = (app.whopLink || "").split("[SPLIT]"); setFormData({ ...app, whopLink: parts[0] || '', reactSourceCode: parts[1] || '', faq: app.faq || Array.from({ length: 7 }, () => ({ q: '', a: '' })) }); setEditingId(app.id); window.scrollTo(0, 0); };
  const handleSubmit = async (e) => { e.preventDefault(); const payload = { ...formData, id: editingId || String(Date.now()), whopLink: `${formData.whopLink}[SPLIT]${formData.reactSourceCode}` }; try { const res = await fetch(editingId ? `${API_URL}/${editingId}` : API_URL, { method: editingId ? 'PUT' : 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }); if (res.ok) { setFormData(initialForm); setEditingId(null); refreshData(); alert('PROTOCOL SAVED.'); } } catch (err) {} }; 
  ...
  if (!isAuthenticated) return (<div className="min-h-screen bg-[#050505] flex items-center justify-center px-6"><div className="bg-[#0a0a0a] p-12 rounded-[3rem] border border-orange-500/20 max-w-md w-full shadow-2xl text-center"><Fingerprint className="w-12 h-12 text-orange-500 mx-auto mb-8" /><form onSubmit={handleLogin} className="space-y-6"><input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="CORE KEY" className="w-full bg-black border border-white/10 p-4 rounded-2xl text-white outline-none text-center text-[11px] tracking-[0.5em]" /><button type="submit" className="w-full rounded-2xl bg-orange-600 px-6 py-4 font-black uppercase text-[10px] tracking-widest shadow-xl">Authorize</button></form></div></div>);
  return (
    <div className="pt-32 pb-24 px-6 max-w-7xl mx-auto font-sans text-left text-white">
      <form onSubmit={handleSubmit} className="bg-[#0a0a0a] border border-white/5 rounded-[2.5rem] p-8 shadow-2xl space-y-6">
           ...
           <textarea value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} placeholder="Description" className="bg-black border border-white/10 p-4 rounded-xl text-[11px] h-48 w-full outline-none font-mono" />
...
function AppContent({ appsData, refreshData }) {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-zinc-100 flex flex-col font-sans relative pb-20 lg:pb-0 text-left">
      ...
      {/* --- FOTER SA BOLD BELIM SLOVIMA I ZNAKOVIMA --- */}
      <footer className="flex flex-col items-center gap-4 text-center text-zinc-100 font-black italic uppercase text-[9px] tracking-[0.5em] py-6 mt-8" style={{ borderTop: '0.5px solid #f97316' }}>
        <div className="flex items-center gap-6">
            ...
            <a href="#" target="_blank" rel="noopener noreferrer" className="opacity-80 hover:opacity-100 transition-opacity"><img src="https://upload.wikimedia.org/wikipedia/commons/a/a5/Instagram_icon.png" alt="Instagram" className="h-4 w-4 object-contain" /></a>
            <a href="#" target="_blank" rel="noopener noreferrer" className="opacity-80 hover:opacity-100 transition-opacity"><img src="https://cdn-icons-png.flaticon.com/512/3046/3046121.png" alt="TikTok" className="h-4 w-4 object-contain" /></a>
        </div>
        <div>
          © 2026 <span className="text-blue-500 font-black">AI TOOLS</span> <span className="text-orange-500 font-black">PRO SMART</span> <span className="mx-1 text-white font-black">|</span> ALL RIGHTS RESERVED
        </div>
      </footer>
    </div>
  );
